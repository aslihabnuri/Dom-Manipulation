      ...
      summary: [
        {
          ...
          questions: [{"q": "Pikirkan organisasi tempat Anda bekerja (atau bisnis yang Anda kenal baik). Petakan proses input–transformation–output utamanya: apa transformed resources yang dominan — material, informasi, atau pelanggan — dan apa implikasinya bagi cara operasi itu seharusnya dikelola?", "guide": "Mulai dari output intinya, lalu telusuri mundur inputnya. Klasifikasikan dengan Tabel 1.2: dominan material → fokus ke logistik & persediaan; dominan informasi → fokus ke akurasi, kecepatan olah data, dan keamanan; dominan pelanggan → fokus ke visibility, keterampilan kontak staf, dan pengalaman selama proses. Jawaban MBA yang baik menyebut konsekuensi manajerial, bukan sekadar klasifikasi."}, {"q": "\"Semua manajer sesungguhnya adalah operations manager.\" Setujukah Anda? Bangun argumen dengan membedakan 'operations sebagai fungsi' dan 'operations sebagai aktivitas'.", "guide": "Sebagai fungsi: hanya satu departemen yang melayani pelanggan eksternal. Sebagai aktivitas: setiap fungsi (marketing, HR, keuangan) punya proses yang mentransformasi input menjadi output — dan proses itu harus dikelola. Posisi yang kuat: setuju dengan catatan — prinsip OM berlaku universal, tapi akuntabilitas fungsi operasi tetap berbeda."}, {"q": "OM sering disebut fungsi dengan pengaruh terbesar terhadap profitabilitas. Mengapa perbaikan 5% pada biaya operasi sering berdampak lebih besar daripada kenaikan 5% penjualan?", "guide": "OM adalah bagian termahal organisasi — sebagian besar biaya ada di sana. Penghematan biaya jatuh langsung ke laba (margin), sedangkan tambahan penjualan masih dipotong biaya variabel untuk melayaninya. Hitung ilustrasi sederhana dengan margin 10% untuk memperkuat argumen."}],
...
berlaku di semua fungsi bisnis — bukan hanya di pabrik.</div>`
        ...
        {
          heading: "The Four Vs",
          questions: [{"q": "Petakan dua bisnis yang Anda kenal — misalnya warteg vs fine dining, atau KRL vs taksi premium — pada keempat dimensi V, lalu jelaskan mengapa struktur biaya keduanya berbeda jauh.", "guide": "Tentukan posisi tiap V (rendah/tinggi) untuk masing-masing bisnis, lalu tarik ke implikasi slide typology: volume tinggi → spesialisasi & biaya unit rendah; variety/variation/visibility tinggi → kompleksitas & biaya unit tinggi. Tutup dengan poin kunci: keduanya bisa untung selama desain operasi konsisten dengan posisinya."}, {"q": "Sebuah butik online ingin menurunkan biaya tanpa kehilangan kesan eksklusif. Dimensi V mana yang paling bisa 'digeser', dan apa risikonya?", "guide": "Kandidat: turunkan visibility untuk proses belakang (pisahkan front office yang eksklusif dari back office yang efisien), kelola variation lewat pre-order/promo off-peak, kurangi variety tersembunyi lewat modularisasi (opsi terlihat banyak, komponen sedikit). Risiko: persepsi eksklusivitas turun bila pemangkasan terlihat pelanggan."}, {"q": "hotelF1 dan Ski Verbier Exclusive sama-sama menguntungkan padahal posisinya bertolak belakang. Apa artinya bagi klaim \"operasi berbiaya rendah selalu lebih baik\"?", "guide": "Klaim itu keliru: yang menentukan bukan rendahnya biaya, melainkan fit antara posisi 4V, strategi, dan harga. Ski Verbier menutup biaya tinggi dengan premium price untuk segmen yang menghargainya. Kaitkan dengan konsep order winners: bagi tamu Ski Verbier, harga bukan winner — pengalaman bespoke-lah yang memenangkan order."}],
          source: { kind: "ppt", label: "Slide Pertemuan 1" },
          visuals: ["fourVs"],
...
desain operasinya konsisten dengan posisinya.</div>`
        },
        {
          ...
          questions: [{"q": "Ambil satu dari 10 keputusan kritis OM dan tunjukkan bagaimana keputusan itu dieksekusi secara berbeda untuk pabrik air minum dalam kemasan vs rumah sakit.", "guide": "Contoh dengan lokasi: pabrik AMDK dekat sumber air & jalur distribusi (dekat bahan baku), rumah sakit dekat populasi pasien (dekat pelanggan). Atau persediaan: AMDK menimbun stok jadi; rumah sakit tidak bisa 'menyimpan' layanan — kapasitasnya yang dikelola. Gunakan tabel goods vs services untuk memperkuat."}, {"q": "Dari enam pergeseran 'from → to' (lokal→global, batch→JIT, dst.), mana yang paling menantang bagi UMKM Indonesia? Mengapa, dan apa langkah pertamanya?", "guide": "Argumen kuat bisa dibangun untuk JIT (butuh pemasok andal & logistik presisi — sulit di infrastruktur kita) atau supply-chain partnering (butuh kepercayaan & kontrak jangka panjang, sementara UMKM terbiasa transaksional). Langkah pertama yang realistis: mulai dari 1–2 pemasok kunci, bukan transformasi total."}],
          source: { kind: "ppt", label: "Slide Pertemuan 1" },
...
seluruh mata kuliah ini.</div>`
        },
        {
          ...
          questions: [{"q": "Era OM ditutup di 'Globalization Focus (2005–2020)'. Menurut Anda, apa nama dan fokus era 2020–2035? Bangun argumen dari the new operations agenda.", "guide": "Kandidat kuat: era 'AI & Resilience Focus' — otomasi cerdas (AI, IoT, robotik), plus pelajaran pandemi: rantai pasok tangguh, reshoring/nearshoring, dan sustainability (triple bottom line jadi regulasi, bukan pilihan). Jawaban bagus menyebut peristiwa pemicu (COVID-19, krisis chip, AI generatif) seperti era-era sebelumnya dipicu inovasinya masing-masing."}, {"q": "Pilih satu teknologi dari agenda baru (IoT, AI, 3D printing, robotik, big data) dan jelaskan dampaknya pada salah satu dari 10 keputusan OM di industri yang Anda kenal.", "guide": "Contoh pola jawaban: IoT → keputusan maintenance (sensor memungkinkan pemeliharaan prediktif, mengurangi downtime); AI → keputusan penjadwalan (optimasi shift & rute); 3D printing → keputusan persediaan & desain (cetak on-demand memangkas stok suku cadang). Sebut juga prasyarat implementasinya: data, keterampilan, proses."}],
          source: { kind: "ppt", label: "Slide Pertemuan 1" },
...
harus terus berdialog dengan marketing, pengembangan produk, dan fungsi-fungsi pendukung.</div>`
        },
        {
          ...
          questions: [{"q": "Bedah Gojek (atau Grab): model bisnis digital apa saja yang dikombinasikannya? Mengapa kombinasi itu saling menguatkan?", "guide": "Minimal empat: On-Demand (transport & kurir), Ecosystem + marketplace (GoFood, GoPay, GoClub — makin banyak dipakai makin bernilai), Subscription (paket langganan), dan elemen Free (data pengguna untuk penargetan). Saling menguatkan karena tiap layanan menurunkan customer acquisition cost layanan lain dan datanya terkonsolidasi — persis pola Walmart di slide."}, {"q": "Sebuah distributor bahan bangunan keluarga ingin 'go digital'. Sebagai konsultan, model apa yang Anda rekomendasikan lebih dulu, dan bagaimana menghindari jebakan 'memasang teknologi tanpa transformasi'?", "guide": "Rekomendasi bertahap: mulai dari digitalisasi proses inti (order & stok) sebelum memilih model pendapatan baru; e-commerce B2B sendiri atau bergabung ke marketplace. Kuncinya dari slide: transformasi digital menuntut perubahan proses, budaya, dan peran karyawan — kalau hanya membeli aplikasi, lima syarat digital business model (CX, profit, share, proses, karyawan) tidak akan terpenuhi."}, {"q": "Model 'Free' menjadikan pelanggan sebagai 'produk' — dan menuai kritik privasi. Sebagai calon pemimpin bisnis, bagaimana Anda menyeimbangkan monetisasi data dengan kepercayaan pelanggan?", "guide": "Kaitkan dengan reputational risk management di new operations agenda: pelanggaran kepercayaan menghancurkan nilai jangka panjang. Kerangka jawaban: transparansi & consent, minimasi data, kepatuhan regulasi (UU PDP), dan uji 'headline test' — apakah praktik data kita aman bila jadi berita utama besok?"}],
          source: { kind: "ppt", label: "Slide Pertemuan 1" },
...
(contoh Walmart), dan gagal bila hanya "memasang alat baru" tanpa mengubah proses dan orangnya.</div>`
        },
        {
          ...
          questions: [{"q": "Bandingkan bank digital (mis. Jago/blu) dengan bank BUMN konvensional pada kelima performance objectives. Gambar polar diagram-nya secara mental — di mana keduanya paling berbeda?", "guide": "Bank digital menonjol di speed (buka rekening menit-an) dan cost (tanpa cabang); bank konvensional di dependability & quality persepsi nasabah senior, plus jaringan fisik sebagai flexibility. Poin tajam: keduanya melayani segmen berbeda, sehingga profil polar yang berbeda justru sama-sama tepat."}, {"q": "\"Dalam jangka panjang, dependability bisa mengalahkan semua kriteria lain.\" Berikan satu contoh nyata di Indonesia yang mendukung atau membantah pernyataan ini.", "guide": "Contoh mendukung: transformasi KRL Commuter Line — ketepatan jadwal mengubah persepsi & loyalitas walau harga naik; atau kurir (pengguna pindah ke ekspedisi yang konsisten walau lebih mahal). Bila membantah: pasar yang sangat sensitif harga. Sebut mekanismenya: dependability dinilai SETELAH pembelian, lalu menumpuk jadi reputasi."}, {"q": "Atasan Anda menuntut layanan lebih cepat DAN lebih murah sekaligus. Gunakan konsep trade-off dan efficient frontier untuk menstrukturkan jawaban profesional Anda.", "guide": "Tiga kemungkinan posisi: (1) kita belum di frontier → keduanya bisa diperbaiki bersamaan lewat menghilangkan pemborosan (contoh Starbucks: lebih cepat SEKALIGUS lebih murah); (2) sudah di frontier → mempercepat berarti menambah biaya, tunjukkan trade-off-nya secara eksplisit; (3) investasi (teknologi/proses baru) menggeser frontier itu sendiri. Jawaban MBA: diagnosis dulu posisi kita, baru pilih strategi."}],
          source: { kind: "ppt", label: "Slide Pertemuan 1" },
...
biaya). Prioritasnya berbeda antar-operasi (polar diagram) dan saling tarik-menarik (trade-off, efficient frontier).</div>`
        },
        {
          ...
          questions: [{"q": "Sebuah klinik membeli sistem antrean & rekam medis digital seharga signifikan. Rancang evaluasi produktivitasnya ala Collins Title: data apa yang Anda kumpulkan, dan mengapa multifactor productivity lebih jujur daripada labor productivity di kasus ini?", "guide": "Data: pasien terlayani/hari (output), jam kerja staf, biaya gaji, dan SEMUA biaya baru (langganan sistem, listrik, pelatihan, depresiasi). Labor productivity bisa naik tajam padahal multifactor hampir datar bila biaya sistem besar — persis pelajaran Collins (75% vs 26%). Sebut juga dimensi kualitas layanan yang tak tertangkap angka."}, {"q": "Mengapa produktivitas sektor jasa lebih sulit ditingkatkan daripada manufaktur — dan bagaimana Starbucks membuktikan bahwa 'sulit' bukan berarti 'mustahil'?", "guide": "Hubungkan dengan karakteristik jasa: knowledge-based, unik per pelanggan, interaksi tinggi, sulit diotomasi, kualitas subjektif. Starbucks menyiasatinya dengan memburu detik pada elemen yang TIDAK dirasakan sebagai pengurangan layanan (tanda tangan, scoop, mesin) — perbaikan proses mikro yang terakumulasi jadi 27%."}],
          source: { kind: "ppt", label: "Slide Pertemuan 1" },
...
persentase perubahan dengan (baru − lama) ÷ lama.</div>`
        },
        {
          ...
          questions: [{"q": "Perusahaan Anda akan menyusun strategi operasi. Rumuskan satu pertanyaan diagnostik kunci untuk masing-masing dari empat perspektif, lalu tunjukkan bagaimana jawabannya bisa saling bertentangan.", "guide": "Top-down: 'Apa yang dituntut strategi korporat dari operasi kita?' Outside-in: 'Apa order winners di mata pelanggan?' Bottom-up: 'Apa yang dipelajari tim lapangan setahun terakhir?' Inside-out: 'Kapabilitas unik apa yang kita punya?' Konflik klasik: pasar minta murah (outside-in) padahal kapabilitas kita di kustomisasi premium (inside-out) — strategi yang baik merekonsiliasinya seperti contoh Micraytech."}, {"q": "Tentukan order winners, order qualifiers, dan less important factors untuk tiga bisnis ini: (a) SPBU, (b) program MBA, (c) marketplace online. Apa implikasi investasinya?", "guide": "Contoh: SPBU — qualifier: takaran akurat & keamanan; winner: lokasi & kecepatan antrean. MBA — qualifier: akreditasi; winner: reputasi alumni & jejaring. Marketplace — qualifier: keamanan pembayaran; winner: harga & kecepatan kirim. Implikasi dari kurvanya: investasi besar hanya untuk winners; qualifiers cukup sampai level lolos — melebihinya adalah pemborosan."}, {"q": "Terapkan importance–performance matrix pada unit kerja Anda: pilih 5–6 faktor kompetitif, skor kepentingan & kinerjanya (1–9), dan tentukan apa yang masuk zona urgent-action.", "guide": "Ikuti pola YIR Laboratories: daftar faktor dari sudut pandang PELANGGAN internal/eksternal (bukan asumsi manajemen), beri dua skor per faktor, petakan. Faktor penting-tapi-kalah-dari-pesaing = urgent action → rencana perbaikan segera; faktor excess → kandidat realokasi sumber daya. Nilai jawaban ada di kejujuran skornya."}],
          source: { kind: "ppt", label: "Slide Pertemuan 1" },
...
Alat analisisnya: operations strategy matrix (line of fit) dan importance–performance matrix (4 zona prioritas).</div>`
        },
        {
          ...
          questions: [{"q": "Indomie dijual di 100+ negara. Analisis: strategi keunggulan mana yang dimainkannya (differentiation, cost, response), dan alasan globalisasi mana dari enam alasan yang paling menjelaskan ekspansinya?", "guide": "Kombinasi cost leadership (skala produksi raksasa) + differentiation rasa lokal per pasar (varian khusus Nigeria!). Alasan globalisasi: memahami pasar, memperpanjang product life cycle (pasar baru saat domestik jenuh), dan memperbaiki supply chain (pabrik di Afrika mendekati pasar). Contoh pola soal ujian: satu perusahaan, banyak konsep."}, {"q": "Rupiah melemah 15% terhadap dolar. Jelaskan bagaimana operational hedging melindungi perusahaan manufaktur yang mengimpor bahan baku sekaligus mengekspor produk jadi.", "guide": "Operational hedging = kapasitas di beberapa negara + kemampuan menggeser produksi mengikuti biaya & kurs (contoh Komatsu vs Caterpillar). Untuk kasus ini: natural hedge (pendapatan dolar menutup biaya impor dolar), relokasi sebagian produksi/pembelian ke zona mata uang lemah, kontrak pemasok multi-negara. Bedakan dengan hedging finansial murni."}],
          source: { kind: "ppt", label: "Slide Pertemuan 1" },
...
globalisasi: supply chain, biaya &amp; kurs, perbaikan operasi, pemahaman pasar, perbaikan produk, dan talenta.</div>`
        },
        {
          ...
          questions: [{"q": "Mana yang lebih menantang dikelola operasinya: pabrik sepatu atau universitas? Gunakan minimal lima dari sembilan perbedaan goods vs services sebagai pisau analisis.", "guide": "Universitas: output intangible, diproduksi-dikonsumsi bersamaan (kuliah), unik per mahasiswa, interaksi tinggi, kualitas sulit diukur, tak bisa disimpan — sehingga kapasitas & penjadwalan jauh lebih rumit dan kualitas bergantung pada orang. Pabrik unggul dalam keterukuran tapi menghadapi tantangan persediaan & logistik. Kesimpulan yang matang: 'menantang' dengan cara yang berbeda."}, {"q": "Kontribusi manajemen terhadap kenaikan produktivitas (±52%) melebihi labor dan capital digabung. Sebutkan tiga keputusan manajerial konkret yang menaikkan produktivitas tim Anda TANPA investasi modal besar.", "guide": "Contoh yang bisa dipertahankan: (1) merancang ulang proses/menghapus langkah tak bernilai tambah (ala Starbucks), (2) penjadwalan yang mencocokkan kapasitas dengan pola permintaan, (3) pelatihan & standardisasi cara kerja terbaik, (4) pemanfaatan data untuk keputusan harian. Poinnya: manajemen = memastikan labor dan capital dipakai efektif."}],
      ...
      summary: [
        {
          ...
          questions: [{"q": "Supply chain global 787 memberi Boeing keunggulan — tapi program itu juga terkenal molor bertahun-tahun dan berbiaya bengkak karena masalah koordinasi mitra. Evaluasilah: kapan strategi 'orkestra global' seperti ini tepat, dan kapan menjadi bumerang?", "guide": "Keunggulan: akses keahlian terbaik dunia, berbagi risiko & modal, membuka pasar negara mitra. Bumerang bila: kompleksitas koordinasi melebihi kapabilitas integrasi, kualitas mitra tak seragam, dependency pada pemasok tunggal. Syarat sukses: kapabilitas integrasi sistem, standar kualitas ketat, dan visibilitas rantai pasok. Jawaban seimbang > jawaban memihak."}, {"q": "Perusahaan Indonesia mana yang paling dekat dengan definisi MNC? Uji kandidat Anda dengan kriteria bukunya, bukan sekadar 'punya ekspor'.", "guide": "Kriteria MNC: keterlibatan internasional EKSTENSIF — membeli sumber daya, berproduksi, DAN menjual di banyak negara, dengan fasilitas dimiliki/dikendalikan lintas negara. Kandidat: Indofood (pabrik di Afrika & Timur Tengah), Mayora, Wings, GoTo (operasi regional). Bandingkan dengan benchmark IBM di buku untuk kalibrasi 'ekstensif'."}],
...
pasar. Bedakan international business (terlibat lintas negara) dengan MNC (keterlibatan ekstensif).</div>`
        },
        {
          ...
          questions: [{"q": "Ambil mission statement perusahaan Anda (atau susun bila belum ada). Kritisi dengan standar bab ini: apakah ia memberi batasan & fokus? Lalu turunkan menjadi mission fungsi operasi satu tingkat di bawahnya.", "guide": "Bandingkan dengan pola Merck/PepsiCo/Arnold Palmer: menyebut untuk siapa organisasi ada, nilai apa yang dijanjikan, dan bagi stakeholder mana. Mission yang buruk: generik, bisa dipakai perusahaan mana pun. Turunannya harus operasional: mis. mission operasi = 'menyediakan X dengan kualitas Y pada biaya Z' yang jelas mendukung mission korporat — tunjukkan garis hierarkinya."}, {"q": "Bisakah satu perusahaan mengejar differentiation DAN cost leadership sekaligus, ataukah pasti 'stuck in the middle'? Ambil posisi dan bela dengan contoh nyata.", "guide": "Buku memberi amunisi: manajer 'more likely to implement some combination' dari tiga strategi. Contoh kombinasi yang berhasil: IKEA (murah + desain khas), Uniqlo (murah + kualitas bahan). Syaratnya: teknologi & desain operasi yang membuat diferensiasi justru menurunkan biaya (activity system yang koheren) — bukan sekadar 'ingin dua-duanya'."}],
          source: { kind: "book", label: "Buku HRM bab 2" },
...
leadership, response — dan operations manager yang menerjemahkannya jadi tugas nyata.</div>`
        },
        {
          ...
          questions: [{"q": "Lakukan analisis five forces singkat untuk industri kopi kekinian Indonesia (Kopi Kenangan, Fore, dkk.). Apa implikasi hasilnya bagi strategi OPERASI para pemainnya?", "guide": "Rivalitas: sangat tinggi; entry barrier: rendah (modal kecil bisa masuk); substitusi: banyak (kopi saset, warkop); daya tawar pelanggan: tinggi (pindah tanpa biaya); pemasok: sedang. Implikasi operasi: cost & speed jadi krusial (perang harga), konsistensi kualitas antar-gerai (standardisasi), dan lokasi. Ini pola soal MBA klasik: analisis industri → turunan operasional."}, {"q": "Produk andalan perusahaan Anda memasuki fase maturity. Tuliskan memo satu paragraf kepada direksi: tiga perubahan operasional yang harus disiapkan sekarang.", "guide": "Dari tabel fase: (1) kontrol biaya menjadi kritis — efisiensi proses & negosiasi ulang pemasok; (2) standardisasi & produksi jangka panjang — kunci desain, kurangi variasi; (3) pertahankan posisi lewat dependability & harga — bukan lagi lewat fitur baru. Bonus: siapkan produk penerus karena decline menyusul. Format memo melatih komunikasi eksekutif."}],
          source: { kind: "book", label: "Buku HRM bab 2" },
          visuals: ["plc"],
...
posisi produk di life cycle mengubah prioritas operasi — strategi yang baik selalu dinamis.</div>`
        },
        {
          ...
          questions: [{"q": "Buat activity map ala Southwest untuk satu bisnis lokal yang Anda kagumi (mis. warung padang legendaris, kedai kopi, atau bengkel langganan): apa keunggulan intinya, dan aktivitas apa saja yang saling mengunci menopangnya?", "guide": "Ikuti struktur Southwest: satu competitive advantage di tengah (mis. 'murah & cepat'), lalu 4–6 aktivitas pendukung yang SALING menguatkan (menu terbatas → belanja bahan efisien → masak batch → penyajian instan). Uji ketajamannya: kalau kompetitor meniru satu aktivitas saja, apakah keunggulannya ikut tersalin? Kalau tidak — itu tanda sistemnya yang unggul."}, {"q": "Apa core competency SEJATI perusahaan Anda? Uji jawaban Anda: apakah itu benar-benar sesuatu yang Anda kerjakan setara/lebih baik dari kompetitor — dan sulit ditiru?", "guide": "Jebakan umum: menyebut produk (bisa ditiru) atau aset (bisa dibeli) alih-alih kapabilitas. Core competency yang kuat biasanya berupa sistem: budaya layanan, kecepatan pengembangan, jaringan distribusi, penguasaan data pelanggan. Kaitkan dengan KSF: apakah kompetensi itu relevan dengan faktor yang menentukan kemenangan di industri Anda?"}],
          source: { kind: "book", label: "Buku HRM bab 2" },
          visuals: ["swot"],
...
Keunggulan yang tahan lama datang dari <em>sistem aktivitas</em>, bukan satu trik.</div>`
        },
        {
          ...
          questions: [{"q": "Sebuah bank ingin meng-outsource call center-nya. Susun analisis untuk direksi: risiko apa saja yang harus diwaspadai (gunakan temuan bab ini), dan rancang tabel factor-rating dengan 5 kriteria berbobot untuk memilih vendornya.", "guide": "Risiko dari buku: ±50% kontrak outsourcing gagal karena perencanaan buruk; skor kepuasan pelanggan TURUN saat customer service di-outsource (domestik maupun luar negeri) — padahal layanan adalah titik kontak nasabah. Kriteria rating: biaya, kualitas layanan/SLA, keamanan data (krusial untuk bank), skalabilitas, kecocokan budaya. Tetapkan bobot, skor 1–5, kalikan-jumlahkan — dan pertimbangkan opsi TIDAK outsourcing untuk aktivitas yang dekat dengan core."}, {"q": "Gelombang reshoring menguat di AS dan Eropa. Ancaman atau peluang bagi Indonesia sebagai negara tujuan outsourcing/manufaktur? Posisikan jawaban Anda dengan theory of comparative advantage.", "guide": "Ancaman: pekerjaan perakitan padat karya kembali ke negara maju via otomasi (comparative advantage upah murah terkikis robot). Peluang: reshoring dari Tiongkok mencari alternatif (China+1) — Indonesia bisa merebutnya bila keunggulan relatifnya digeser dari 'murah' ke 'terampil + pasar besar + stabil'. Jawaban tajam menyebut kebijakan yang diperlukan: infrastruktur, keterampilan, kepastian hukum."}],
          source: { kind: "book", label: "Buku HRM bab 2" },
...
semua biaya, dan pakai factor-rating method agar keputusan objektif. Tren balik arah: reshoring.</div>`
        },
        {
          ...
          questions: [{"q": "Klasifikasikan ke dalam matriks empat strategi global: (a) McDonald's, (b) Toyota, (c) Kopi Kenangan bila ekspansi ke Malaysia & Singapura. Bela penempatan Anda.", "guide": "McDonald's: berawal multidomestic (menu lokal: McRendang?) bergerak ke transnational (standar global + adaptasi lokal). Toyota: transnational — skala & standardisasi global dengan produksi lokal responsif. Kopi Kenangan: kemungkinan multidomestic di awal (selera & harga lokal) — atau international bila sekadar lisensi/waralaba. Yang dinilai bukan 'jawaban benar' tapi kualitas argumen pada dua sumbu: cost reduction vs local responsiveness."}, {"q": "Kapan sebuah perusahaan sebaiknya BERPINDAH kuadran strategi global? Ceritakan skenario perpindahan dari international strategy menuju transnational — apa pemicunya dan kapabilitas apa yang harus dibangun di tiap tahap?", "guide": "Alur umum: international (ekspor/lisensi — uji pasar, risiko kecil) → multidomestic ATAU global (saat volume membenarkan investasi lokal / saat tekanan biaya menuntut skala) → transnational (saat keduanya menekan bersamaan). Pemicu: pertumbuhan pasar, tekanan harga kompetitor, tuntutan adaptasi lokal. Kapabilitas: dari sekadar logistik ekspor → manajemen anak perusahaan → koordinasi & pembelajaran lintas negara. Ingat: transnational paling sulit — jangan lompat ke sana tanpa fondasi."}],
          source: { kind: "book", label: "Buku HRM bab 2" },
