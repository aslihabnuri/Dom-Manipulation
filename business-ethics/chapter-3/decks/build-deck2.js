...
// ============================================================ 02 index
{
  ...
  const idx = [
    ['01', 'Kerangka Analisis', 'Alasan satu teori tidak memadai untuk kasus ini'],
    ['02', 'Profil Oil Sands', 'Objek, skala, dan aktor industrinya'],
    ['03', 'Peta Pemangku Kepentingan', 'Enam pihak dengan kepentingan berbenturan'],
    ...
    ['08', 'Matriks Putusan', 'Sembilan penilaian atas fakta yang identik'],
...
// ============================================================ 03 kerangka analisis
{
  ...
  s.addText('Kasus direduksi menjadi satu pertimbangan normatif. Kesimpulannya tegas, namun keberatan pihak lain tidak terakomodasi.', {
  ...
  s.addText('Setiap teori menyoroti dimensi yang luput dari teori lain. Yang dicari bukan jawaban tunggal, melainkan pertimbangan yang paling kuat menanggung beban argumen.', {
    isTextBox: true, x: px + 0.3, y: 3.18, w: w2 - 0.66, h: 1.16, fontFace: F, fontSize: 12.5, color: 'E8E4DC', margin: 0, lineSpacing: 18
  });

  s.addText('Sembilan lensa analisis', { isTextBox: true, x: M, y: 4.82, w: CW, h: 0.3, fontFace: F, fontSize: 12, bold: true, color: SLATE, margin: 0 });
...
// ============================================================ 04 profil kasus
{
  ...
  const st = [
    { v: '140', u: 'rb km²', l: 'Luas deposit di Alberta dengan Fort McMurray sebagai pusat industri', fill: ORNG },
    { v: '2,77', u: 'juta', l: 'Barel per hari pada 2017, naik dari 0,5 juta barel pada 1997', fill: LAV },
    { v: '170', u: 'M barel', l: 'Cadangan terbukti Kanada, terbesar ketiga secara global', fill: TEAL },
    { v: '99', u: '%', l: 'Ekspor minyak Kanada dengan tujuan Amerika Serikat', fill: LIME }
  ];
  ...
  const tl = [
    ...
    ['1960-an', 'Operasi komersial dimulai, terhambat biaya ekstraksi yang tinggi'],
    ['2000-an', 'Lonjakan harga minyak menarik masuk Shell, Chevron, Total, Exxon, dan CNOOC'],
    ['2012', 'Investasi tahunan mencapai puncak di atas 30 miliar dolar Amerika'],
    ['2018', 'Investasi terkoreksi dua pertiga menjadi sekitar 10 miliar dolar per tahun']
  ];
  s.addText('Kronologi pengembangan industri', { isTextBox: true, x: M, y: 4.4, w: CW, h: 0.3, fontFace: F, fontSize: 12, bold: true, color: SLATE, margin: 0 });
  tl.forEach((t, i) => {
    const y = 4.76 + i * 0.42;
...
// ============================================================ 05 pemangku kepentingan
{
  ...
  sub(s, 'Setiap teori berikutnya membobot keenam pihak ini secara berbeda.', 1.62);

  const pk = [
    [ORNG, 'Pemerintah Alberta', 'Penerimaan pajak, penyerapan tenaga kerja, dan kedaulatan energi provinsi.'],
    [LAV, 'Perusahaan Minyak', 'Suncor, Syncrude, Shell, Chevron, Total, Exxon, dan CNOOC.'],
    [TEAL, 'Pekerja Fort McMurray', 'Penghidupan langsung bagi 206.000 pekerja beserta keluarganya.'],
    [AMBR, 'Masyarakat Adat Hilir', 'Sumber air, perikanan, dan tanah adat sepanjang Sungai Athabasca.'],
    [INK, 'Organisasi Lingkungan', 'Sierra Club, Greenpeace, Environmental Defence, Pembina Institute.'],
    ['6C7BE0', 'Generasi Mendatang', 'Menanggung akibat iklim tanpa keterwakilan dalam proses pengambilan keputusan.']
  ];
  ...
  banner(pres, s, M, 6.34, CW, 0.5, 'Dua pihak terakhir tidak terwakili dalam proses pengambilan keputusan. Inilah kelemahan struktural kasus ini.', AMBR, INK, 11.5);
...
// ============================================================ 06 dua narasi
{
  ...
  const neg = [
    ['Emisi', '20 persen di atas minyak konvensional, 10 persen emisi nasional Kanada'],
    ['Limbah', 'Kolam beracun 220 kilometer persegi berisi sekitar 1 triliun liter, terindikasi bocor'],
    ['Habitat', 'Hutan boreal seluas Kota New York hilang, reklamasi baru mencapai 11 persen'],
  ...
  const pos = [
    ...
    ['Tata Kelola', 'Tata kelola demokratis dan bebas korupsi, berbeda dari produsen pembanding']
  ];
  pos.forEach((n, i) => {
    const y = 3.42 + i * 0.62;
  ...
  banner(pres, s, M, 6.06, CW, 0.78, 'Kedua kolom merujuk data resmi yang sama sahnya. Sengketanya bukan sengketa fakta, melainkan sengketa kriteria penilaian.', INK, W, 12.5);
...
// ============================================================ 07 lensa akibat
{
  ...
  lens(s, {
    ...
    q: 'Apakah kebijakan ini melayani kepentingan jangka panjang Kanada dan korporasinya?',
    b: 'Pada horizon pendek, jawabannya afirmatif. Penerimaan pajak, penyerapan tenaga kerja, dan kedaulatan energi seluruhnya meningkat.\n\nPersoalannya terletak pada horizon panjang. Biaya iklim tidak terinternalisasi ke dalam harga karena generasi penanggungnya absen dari pasar. Inilah market failure yang diakui egoism sebagai batas teorinya.',
  ...
  lens(s, {
    ...
    b: 'Act utilitarianism cenderung mendukung. Manfaat 206.000 lapangan kerja bersifat nyata dan terukur, sedangkan kerugiannya tersebar dan sulit dikuantifikasi.\n\nRule utilitarianism menolak. Bila kelas tindakan ini menjadi aturan umum, target Paris menjadi mustahil dan kerugiannya melampaui manfaat mana pun.',
    v: 'Terbelah'
  });
...
// ============================================================ 08 lensa prinsip
{
  ...
  lens(s, {
    ...
    q: 'Dapatkah prinsip yang dianut Kanada diuniversalkan ke seluruh negara?',
    b: 'Kanada memegang cadangan terbukti terbesar ketiga. Bila Venezuela, Arab Saudi, Iran, dan Irak menerapkan prinsip identik, tidak tersisa anggaran karbon global.\n\nMaksim tersebut gagal diuniversalkan. Kegagalan ini memadai untuk membatalkannya menurut Kant.',
    v: 'Menolak'
  });
  lens(s, {
    i: 4, x: M + w3 + 0.24, y: 2.16, w: w3, h: 4.3,
    q: 'Hak siapa yang wajib dipertimbangkan, dan apakah martabatnya dihormati?',
    b: 'Pasal 23 UDHR mendukung: hak atas pekerjaan dan kondisi kerja yang adil terpenuhi bagi 206.000 pekerja.\n\nSebaliknya, hak atas kesehatan komunitas hilir Athabasca dan hak atas tanah adat terlanggar. Prinsip Ruggie menuntut penghormatan hak pada seluruh relasi bisnis.',
    v: 'Mendukung bersyarat'
  });
  lens(s, {
    i: 5, x: M + 2 * (w3 + 0.24), y: 2.16, w: w3, h: 4.3,
    q: 'Prinsip apa yang dipilih tanpa mengetahui posisi sosial yang akan ditempati?',
    b: 'Manfaat tersebar secara nasional, sedangkan beban terkonsentrasi pada komunitas hilir dan generasi mendatang.\n\nKriteria pertama Rawls tidak terpenuhi: kebebasan dasar atas air bersih dan kesehatan tidak merata, sehingga ketimpangan berikutnya tidak dapat dibenarkan.',
    v: 'Menolak'
  });
...
// ============================================================ 09 lensa alternatif
{
  ...
  const b = [
    'Pengukuran diri terhadap produsen terburuk merupakan standar terendah. Aktor berkarakter baik mengacu pada standar tertinggi.',
    'Komunitas hilir dan generasi mendatang merupakan pihak berelasi, bukan variabel biaya. Yang dituntut transisi yang adil, bukan penghentian mendadak.',
    'Label ethical oil lahir dari kampanye berpendanaan industri. Syarat imparsialitas dan bebas manipulasi persuasif tidak terpenuhi.',
    'Oil sands berkonotasi netral, tar sands berkonotasi merusak. Perebutan istilah menunjukkan bahasa turut membentuk penilaian moral.'
  ];
...
// ============================================================ 10 matriks putusan
{
  ...
  const rows = [
    ['Egoism', 'Biaya iklim tidak terinternalisasi karena generasi mendatang absen dari pasar', 'Netral'],
    ['Utilitarianism', 'Act mendukung manfaat kini, rule menolak karena target Paris menjadi mustahil', 'Terbelah'],
    ['Ethics of Duty', 'Prinsipnya gagal diuniversalkan pada seluruh pemilik cadangan terbesar', 'Menolak'],
    ['Ethics of Rights', 'Hak atas pekerjaan terpenuhi, hak atas kesehatan dan tanah adat terlanggar', 'Bersyarat'],
    ['Justice', 'Manfaat tersebar, beban terkonsentrasi. Kriteria pertama Rawls tidak terpenuhi', 'Menolak'],
    ['Virtue Ethics', 'Pengukuran diri terhadap produsen terburuk merupakan standar terendah', 'Menolak framing'],
    ['Ethic of Care', 'Komunitas hilir dan generasi mendatang merupakan relasi, bukan variabel biaya', 'Transisi adil'],
    ['Discourse Ethics', 'Label ethical oil lahir dari kampanye berpendanaan industri, imparsialitas gagal', 'Menolak proses'],
    ['Postmodern Ethics', 'Perebutan istilah oil sands dan tar sands menunjukkan bahasa membentuk moral', 'Anti absolut']
  ];
  rows.forEach((r, i) => {
    const y = 1.88 + i * 0.54;
  ...
  s.addText('Tidak satu pun berbunyi mendukung tanpa syarat. Enam menolak, dua bersyarat, satu netral karena mengakui batas teorinya sendiri.', {
...
// ============================================================ 11 menguji ethical oil
{
  ...
  const pr = [
    [ORNG, 'Perbandingan Komparatif Tidak Bernilai Normatif', 'Keburukan Venezuela dan Arab Saudi tidak membenarkan tindakan Kanada. Virtue ethics mengategorikannya sebagai pengukuran pada standar terendah.'],
    [LAV, 'Cacat Prosedural Sejak Awal', 'Label ethical oil lahir dari kampanye berpendanaan industri. Discourse ethics membatalkannya karena syarat imparsialitas tidak terpenuhi.'],
    [TEAL, 'Distribusi Manfaat dan Beban Tidak Simetris', 'Manfaat tersebar nasional, beban terkonsentrasi pada komunitas hilir dan generasi mendatang. Rawls menolak susunan distributif semacam ini.'],
    [INK, 'Prinsipnya Gugur Ketika Diuniversalkan', 'Bila seluruh pemilik cadangan terbesar bertindak identik, tidak tersisa anggaran karbon global. Uji Kant gagal pada titik ini.']
  ];
  pr.forEach((p, i) => numRow(pres, s, {
    x: M, y: 2.16 + i * 1.06, w: CW, h: 0.96, n: i + 1, fill: p[0], t: p[1], b: p[2]
  }));

  banner(pres, s, M, 6.44, CW, 0.5, 'Penolakan atas klaim ethical oil tidak setara dengan tuntutan penghentian operasi. Keduanya persoalan terpisah.', LIME, INK, 11.5);
...
// ============================================================ 12 implikasi
{
  ...
  s.addText('Struktur dilemanya nyaris identik. Manfaat ekonominya besar dan terukur, biaya lingkungan dan sosialnya tersebar dan sulit dikuantifikasi. Narasi pembenarnya setara: nikel Indonesia dibutuhkan bagi transisi energi global.', {
    isTextBox: true, x: M + 0.32, y: 5.64, w: CW - 0.68, h: 0.52, fontFace: F, fontSize: 12, color: 'B9C0CE', margin: 0, lineSpacing: 17
  });
  s.addText('Apakah narasi nikel yang etis lolos dari kerangka uji yang sama?', {
...
// ============================================================ 13 kesimpulan
{
  ...
  const tm = [
    [ORNG, 'Konvergensi, Bukan Kesepakatan', 'Sembilan teori berangkat dari premis berbeda, namun enam di antaranya menghasilkan arah yang sama. Konvergensi dari titik masuk berbeda inilah bukti terkuat yang dapat dihasilkan pluralism.'],
    [LAV, 'Klaimnya yang Gugur', 'Yang gugur bukan industrinya, melainkan klaim bahwa industri ini etis. Persoalan transisi, kompensasi, dan reklamasi tetap terbuka.'],
    [INK, 'Prisma Bekerja pada Kasus Nyata', 'Lensa tunggal menghasilkan jawaban yang rapi namun menyesatkan. Prisma menghasilkan spektrum yang tidak nyaman, tetapi dapat dipertanggungjawabkan kepada seluruh pihak terdampak.']
  ];
...
// ============================================================ 14 penutup
{
  ...
  s.addText('Sembilan lensa atas satu kasus tanpa jawaban tunggal. Kondisi inilah yang menjadikan kasus ini layak didiskusikan.', {
