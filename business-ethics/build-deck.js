...
s.addText("BUSINESS ETHICS FOR SUSTAINABILITY", {
...
head(s, "PETA BAHASAN", "Sistematika pembahasan", "Kerangka teoretis dibangun lebih dahulu agar analisis kasus memiliki dasar penilaian yang eksplisit.");
card(s, M, 1.88, 5.82, 4.62, SURF);
card(s, M + 6.27, 1.88, 5.82, 4.62, SURF);
label(s, M + 0.34, 2.12, 5.1, "BAGIAN SATU", EMBER);
label(s, M + 6.61, 2.12, 5.1, "BAGIAN DUA", TEAL);
heading(s, M + 0.34, 2.42, 5.1, "Kerangka penilaian", 16);
heading(s, M + 6.61, 2.42, 5.1, "Hilirisasi nikel Indonesia", 16);
const agL = [["01", "Peran teori etika normatif"], ["02", "Absolutisme, relativisme, pluralisme"], ["03", "Klasifikasi sembilan teori"], ["04", "Lima teori modernis Barat"], ["05", "Keterbatasan teori modernis"], ["06", "Empat teori alternatif"]];
const agR = [["07", "Anatomi kebijakan dan sengketanya"], ["08", "Dua narasi atas fakta yang sama"], ["09", "Penilaian lima teori modernis"], ["10", "Penilaian empat teori alternatif"], ["11", "Sintesis penilaian dan evaluasi klaim"], ["12", "Simpulan dan implikasi manajerial"]];
agL.forEach((it, i) => { const yy = 2.94 + i * 0.56;
...
agR.forEach((it, i) => { const yy = 2.94 + i * 0.56;
...
/* ============ 3 PERAN TEORI ============ */
s = pres.addSlide(); bg(s);
head(s, "01   PERAN TEORI ETIKA NORMATIF", "Mengapa teori normatif diperlukan", "Dalam ranah personal intuisi memadai. Dalam konteks bisnis, penilaian etis harus dapat dipertahankan di hadapan pemangku kepentingan.");
card(s, M, 1.96, 5.6, 2.32, INK);
label(s, M + 0.34, 2.20, 5.0, "DEFINISI KONSEP", MUTED_D);
...
const fungsi = [
  ["Rasionalisasi intuisi moral", "Teori etika berfungsi merasionalkan, menjelaskan, dan memahami intuisi moral yang telah dimiliki mengenai benar dan salah."],
  ["Dasar diskusi rasional", "Teori memungkinkan diskursus rasional antarpihak yang berbeda nilai moral, sehingga perbedaan tidak berhenti pada tataran preferensi."],
  ["Justifikasi keputusan bisnis", "Keputusan bisnis menuntut dasar rasional yang sistematis dan dapat dipertanggungjawabkan kepada pemangku kepentingan."]];
...
/* ============ 4 POSISI DASAR ============ */
s = pres.addSlide(); bg(s);
head(s, "02   POSISI DASAR", "Absolutisme, relativisme, dan pluralisme", "Sebelum menerapkan teori, perlu ditetapkan seberapa luas klaim keberlakuan yang diajukan teori tersebut.");
const spek = [
  ["ABSOLUTISME ETIS", "Ada prinsip moral yang universal dan abadi", "Benar dan salah adalah kualitas objektif yang bisa ditentukan lewat nalar, terlepas dari keadaan.\n\nHampir seluruh teori modernis Barat berada pada posisi ini. Keunggulannya terletak pada ketegasan kesimpulan yang dihasilkan.", SURF, INK, EMBER],
  ...
  ["PLURALISME ETIS", "Nilai yang bertentangan dapat sama sahnya", "Posisi yang diambil Crane dan Matten. Pluralisme tidak menyamaratakan seluruh perspektif, tetapi juga tidak mengunggulkan satu perspektif di atas yang lain.\n\nKonflik antarteori bukan kegagalan, melainkan konsekuensi yang harus dikelola.", TEAL_S, TEAL, TEAL]];
const cw3 = (CW - 0.8) / 3;
...
s.addText([
  { text: "Catatan kritis.   ", options: { fontFace: BF, fontSize: 12, bold: true, color: INK } },
  { text: "Sen (2000) menilai pluralisme membuka kepekaan terhadap realitas yang berubah, sesuatu yang tidak disediakan oleh teori tunggal. Liu (2018) sebaliknya mengingatkan risiko toleransi berlebih, sebab sebagian praktik dan tradisi memang tidak layak ditoleransi.", options: { fontFace: BF, fontSize: 12, color: BODY } }
...
/* ============ 5 PETA TEORI ============ */
s = pres.addSlide(); bg(s);
head(s, "03   KLASIFIKASI TEORI", "Klasifikasi sembilan teori etika normatif", "Klasifikasi pertama menilai tindakan. Klasifikasi kedua menilai pelaku, relasi, prosedur, dan bahasa.");
card(s, M, 1.96, 7.55, 4.38, SURF);
label(s, M + 0.34, 2.18, 6.9, "KLASIFIKASI I   |   TEORI MODERNIS BARAT", EMBER);
para(s, M + 0.34, 2.48, 6.9, 0.66, "Berakar pada Pencerahan abad ke-18, bersifat absolutis, dan menawarkan aturan yang dapat diterapkan pada situasi apa pun. Keunggulannya terletak pada solusi yang tidak mendua.", 11.5, BODY, 1.1);
...
para(s, M + 0.34, 5.14, 6.9, 1.05, "Seluruh teori ini bersifat normatif karena berangkat dari asumsi tentang hakikat dunia dan hakikat manusia. Penerimaan atas kesimpulannya bergantung pada penerimaan atas asumsi tersebut.", 11.5, BODY, 1.14);
card(s, M + 7.95, 1.96, 4.14, 4.38, INK);
label(s, M + 8.29, 2.18, 3.5, "KLASIFIKASI II   |   TEORI ALTERNATIF", MUTED_D);
...
s.addText("Cenderung relativis. Lahir dari keberatan atas klasifikasi pertama, dan menyoroti dimensi yang tidak tertangkap oleh aturan universal.", { x: M + 8.29, y: 2.84, w: 3.5, h: 0.92, isTextBox: true, margin: 0, valign: "top", fontFace: BF, fontSize: 11.5, color: PALE2, lineSpacingMultiple: 1.14 });
...
theorySlide({
  ...
  dek: "Teori tertua sekaligus paling diperdebatkan. Sebagian penulis bahkan menolak mengategorikannya sebagai teori moral.",
  ...
  leftLabel: "PEMBEDAAN KONSEPTUAL",
  leftBody: "Egoisme berbeda dari keserakahan. Pelaku egois masih dapat tergerak oleh rasa iba, sedangkan pelaku serakah tidak peka pada kepentingan pihak lain.\n\nEgoisme berbasis kepentingan jangka panjang lebih kuat daripada egoisme berbasis keinginan, sebab hanya yang pertama mampu membedakan kualitas pilihan hidup seseorang.",
  blocks: [
    ["Tokoh dan karya utama", "Thomas Hobbes dalam Leviathan, yang menyebut keadaan alamiah sebagai perang semua melawan semua, dan Ayn Rand dalam The Virtue of Selfishness."],
    ["Enlightened egoism", "Perusahaan menyokong sekolah atau layanan kesehatan setempat bukan atas dasar altruisme, melainkan karena perbaikan lingkungan sosial menguntungkan retensi tenaga kerjanya."],
    ["Keterbatasan teori", "Teori ini hanya berlaku bila pasar mencegah satu pelaku egois merugikan pelaku lain. Ia gugur pada kegagalan pasar dan pada isu keberlanjutan, sebab korban penipisan sumber daya adalah generasi mendatang."]],
...
/* ============ 7 UTILITARIANISM ============ */
s = pres.addSlide(); bg(s);
head(s, "04   TEORI MODERNIS BARAT   |   KONSEKUENSIALIS", "Etika hasil: utilitarianism", "Teori yang paling lazim diterapkan dalam bisnis karena kompatibel dengan metodologi kuantitatif ekonomi.");
...
const ciri = [["Consequentialism", "Objek penilaiannya akibat"], ["Hedonism", "Utilitas sebagai pleasure dikurangi pain"], ["Maximalism", "Jumlah terbesar, bukan sekadar baik"], ["Universalism", "Akibat bagi semua pihak dihitung"]];
...
label(s, M + 8.29, 2.18, 3.5, "EMPAT KEBERATAN UTAMA", EMBER);
const masalah = [
  ["Subjektivitas", "Penilaian pleasure dan pain bergantung pada pihak yang menghitung dan pada cakupan yang diperhitungkan."],
  ["Pembobotan setara", "Kepentingan pelaku dan pihak terdekatnya harus diberi bobot sama dengan pihak jauh di rantai pasok."],
  ["Kuantifikasi", "Hal yang bernilai intrinsik sulit dinilai secara moneter, misalnya pengalaman masa kanak-kanak."],
  ["Distribusi utilitas", "Pengejaran kebaikan terbesar bagi jumlah terbesar mengabaikan minoritas dan generasi mendatang."]];
...
label(s, M + 0.34, 5.16, 5.0, "RELEVANSI BAGI BISNIS", TEAL);
...
label(s, M + 6.39, 5.34, 5.4, "TIGA KEBERATAN", EMBER);
para(s, M + 6.39, 5.62, 5.4, 0.82, "Pertama, kriteria motivasinya terlalu sempit: perawat yang menemani pasien sekarat atas dasar iba tidak dinilai bermoral. Kedua, akibat diabaikan sepenuhnya. Ketiga, asumsi rasionalitas penuh bersifat idealistis.", 11.5, BODY, 1.12);
...
label(s, M + 0.34, 4.12, 5.0, "KORELASI HAK DAN KEWAJIBAN", EMBER);
para(s, M + 0.34, 4.42, 4.95, 2.0, "John Locke merumuskan natural rights atas hidup, kebebasan, dan milik, yang kemudian meluas ke kebebasan berpendapat, hati nurani, persetujuan, privasi, dan proses hukum yang adil.\n\nHak seseorang menimbulkan kewajiban pada pihak lain. Hak atas privasi mewajibkan pihak lain menahan diri dari pengumpulan informasi pribadi tanpa persetujuan.", 11.5, BODY, 1.16);
...
para(s, M + 6.39, 4.54, 5.4, 0.76, "Cakupan tanggung jawab: perusahaan wajib mencegah dan meredakan dampak buruk yang terkait langsung melalui relasi bisnisnya, sekalipun ia tidak ikut menyebabkannya. Pemasok termasuk di dalamnya.", 11.5, "7A3A1C", 1.12);
card(s, M + 6.05, 5.72, 6.04, 0.82, SURF2);
label(s, M + 6.39, 5.88, 5.4, "PASAL YANG PALING RELEVAN BAGI BISNIS", EMBER);
...
/* ============ 10 JUSTICE ============ */
s = pres.addSlide(); bg(s);
head(s, "04   TEORI MODERNIS BARAT   |   BERBASIS PRINSIP", "Keadilan: Rawls dan kontrak sosial", "Pertanyaannya bergeser dari benar atau salah menjadi susunan seperti apa yang layak disepakati bersama.");
card(s, M, 1.96, 5.6, 2.72, INK);
...
para(s, M + 0.26, 5.44, 2.24, 1.0, "Keadilan pada prosesnya. Siapa yang berhak turut memutuskan dan melalui mekanisme apa.", 11.5, BODY, 1.12);
card(s, M + 2.88, 4.98, 2.72, 1.56, SURF);
label(s, M + 3.14, 5.16, 2.2, "DISTRIBUTIF", EMBER);
para(s, M + 3.14, 5.44, 2.24, 1.0, "Keadilan pada hasilnya. Ke mana manfaat mengalir dan siapa yang menanggung bebannya.", 11.5, BODY, 1.12);
...
s = pres.addSlide(); bg(s);
head(s, "05   EVALUASI KRITIS", "Enam keterbatasan teori modernis Barat", "Keberatan ini menjadi dasar kemunculan empat teori alternatif yang dibahas berikutnya.");
const batas = [
...
s.addText("Teori alternatif tidak menggantikan teori modernis, melainkan menambahkan dimensi penilaian yang belum tercakup: karakter pelaku, relasi antarpihak, prosedur perumusan norma, dan pilihan istilah.", {
...
/* ============ 12 VIRTUE DAN CARE ============ */
s = pres.addSlide(); bg(s);
head(s, "06   TEORI ALTERNATIF", "Virtue ethics dan ethic of care", "Objek penilaian bergeser dari tindakan ke karakter pelaku dan ke relasi antarpihak yang terlibat.");
altCard(s, M, 6, "Virtue ethics", "Pelaku seperti apa yang layak disebut baik pada posisi ini?",
  ...
  ["PENERAPAN DALAM BISNIS", "Solomon menempatkan perusahaan sebagai komunitas praktik, dengan keutamaan berupa kejujuran, keadilan, kepercayaan, dan ketangguhan."],
  ["KETERBATASAN", "Keutamaan selalu ditentukan komunitas tertentu, sehingga sulit dijadikan ukuran yang berlaku lintas budaya dan lintas industri."]], false);
altCard(s, M + 6.27, 7, "Ethic of care", "Solusi mana yang memelihara relasi dengan pihak terdampak?",
  ...
  ["TUNTUTAN NORMATIF", "Empati, keselarasan, menghindari kerugian, dan tanggung jawab konkret pada orang tertentu, bukan penerapan aturan yang seragam."],
  ["KETERBATASAN", "Prioritas pada pihak terdekat berisiko mengabaikan pihak jauh, dan kriteria kepedulian sulit diaudit secara seragam."]], true);
...
s = pres.addSlide(); bg(s);
head(s, "06   TEORI ALTERNATIF", "Discourse ethics dan postmodern ethics", "Objek penilaian bergeser ke prosedur perumusan norma dan ke bahasa yang dipakai untuk membenarkannya.");
altCard(s, M, 8, "Discourse ethics", "Norma apa yang bisa lahir dari deliberasi yang terbuka?",
  ...
  ["ORIENTASI", "Penyelesaian konflik secara damai, bukan pembuktian satu kebenaran tunggal. Prosedur yang cacat cukup untuk membatalkan hasilnya."]], false);
altCard(s, M + 6.27, 9, "Postmodern ethics", "Kepentingan siapa yang dilayani oleh pilihan istilah?",
  ...
  ["GAGASAN INTI", "Dorongan moral dan perasaan didahulukan atas perhitungan rasional. Tindakan etis kerap digerakkan oleh intuisi, bukan kalkulasi."],
  ["KRITIK TERHADAP ORGANISASI", "Prosedur, pembagian tugas, dan jarak birokratis menciptakan jarak moral yang menumpulkan dorongan itu."],
  ["IMPLIKASI SIKAP", "Mempertanyakan bahasa dan asumsi yang telah dianggap wajar, lalu bertindak pada skala lokal tempat akibatnya dirasakan."]], true);
...
/* ============ 14 DAFTAR PERIKSA ============ */
s = pres.addSlide(); bg(s);
head(s, "07   SINTESIS KERANGKA", "Sembilan pertanyaan penilaian", "Tidak ada teori yang memberikan jawaban lengkap. Yang dicari bukan jawaban tunggal, melainkan pertimbangan yang paling kuat menanggung beban argumen.");
const tanya = [
  [1, "Egoism", "Apakah semua pihak bebas mengejar kepentingan jangka panjangnya, dan sudahkah biaya tertunda dihitung?", EMBER],
  [2, "Utilitarianism", "Bila seluruh akibat diagregasi, apakah hasilnya positif, dan apakah act dan rule sejalan?", EMBER],
  ...
  [6, "Virtue ethics", "Bagaimana pelaku berkarakter baik bertindak di posisi ini, dan diukur terhadap standar yang mana?", TEAL],
...
s.addText("Pendekatan pluralis yang dianjurkan Bab 3 menghasilkan sembilan penilaian atas rangkaian fakta yang identik, untuk kemudian dilihat arah konvergensinya.", {
...
head(s, "BAGIAN DUA   |   STUDI KASUS INDONESIA", "Anatomi kebijakan hilirisasi nikel", "Satu instrumen regulasi yang mengubah struktur industri nikel nasional sekaligus menimbulkan sengketa di dua arah.", true);
const cwa = (CW - 0.8) / 3;
...
s.addText("Kasus ini dipilih karena strukturnya khas: manfaatnya besar, terukur, dan terkonsentrasi, sedangkan bebannya tersebar, tertunda, dan sulit dikuantifikasi. Persoalan dengan struktur demikian tidak dapat diselesaikan melalui kriteria penilaian tunggal.", {
...
/* ============ 16 DUA NARASI ============ */
s = pres.addSlide(); bg(s);
head(s, "08   FAKTA", "Dua narasi atas fakta yang sama", "Kedua kolom bersandar pada data resmi yang sama sahnya. Yang disengketakan bukan faktanya, melainkan kriteria penilaian yang dipakai.");
...
s = pres.addSlide(); bg(s);
head(s, "09   PENERAPAN", "Penilaian berdasarkan lima teori modernis", "Penilaian berbasis akibat dan berbasis prinsip menghasilkan kesimpulan yang tidak seragam.");
[[1, "Egoism", "NETRAL", MUTED, "Pada horizon pendek jelas melayani kepentingan Indonesia. Pada horizon panjang, biaya kesehatan warga, pemulihan sungai, dan pensiun dini pembangkit batu bara belum diinternalisasi. Kepentingan diri yang dihitung cermat justru menuntut biaya tertunda itu masuk sejak awal."],
 [2, "Utilitarianism", "AMBIVALEN", GOLD, "Act utilitarianism cenderung mendukung, sebab manfaat 166 ribu lapangan kerja nyata dan terukur sedangkan kerugiannya tersebar. Rule utilitarianism menolak, sebab bila setiap negara pemilik cadangan menempuh pengolahan bertenaga batu bara, tambahan emisinya membatalkan manfaat transisi energi yang justru menjadi pembenar kebijakan ini."],
...
foot(s, "Dua teori dalam klasifikasi yang sama menghasilkan kesimpulan berbeda, dan pembedanya terletak pada unit analisis: tindakan tunggal atau kelas tindakan.", 17);
...
s = pres.addSlide(); bg(s);
head(s, "10   PENERAPAN", "Penilaian berdasarkan empat teori alternatif", "Penilaian bergeser ke karakter pelaku, relasi, prosedur, dan bahasa, tanpa mengubah rangkaian faktanya.");
const cw4 = (CW - 0.45) / 2;
[[6, "Virtue ethics", "MENOLAK PEMBANDING", EMBER, "Standar pembanding mana yang seharusnya dipakai?", "Pembandingnya selama ini adalah keadaan sebelum hilirisasi, bukan standar industri terbaik yang tersedia hari ini. Aktor berkarakter baik mengukur diri pada standar tertinggi yang dapat dicapai, bukan pada titik berangkatnya sendiri."],
 [7, "Ethic of care", "MENUNTUT PEMULIHAN", GOLD, "Sejauh mana relasi dengan pihak terdampak dipelihara?", "Warga hilir sungai dan sekitar 300 sampai 500 jiwa O Hongana Manyawa adalah pihak yang berelasi, bukan variabel biaya dalam neraca. Yang dituntut pemulihan sungai dan wilayah lindung, bukan penghentian mendadak yang mencabut penghidupan pekerja."],
 [8, "Discourse ethics", "MENOLAK PROSEDUR", EMBER, "Siapa yang hadir dan siapa yang absen dalam perumusan?", "Warga terdampak bukan pihak dalam perumusan kebijakan maupun perizinan kawasan. Syarat keterlibatan setara dan bebas paksaan tidak terpenuhi sejak awal, dan cacat prosedur sudah cukup untuk membatalkan keabsahan hasilnya."],
 [9, "Postmodern ethics", "RELATIVISTIK", MUTED, "Kepentingan siapa yang dilayani oleh pilihan istilah?", "Hilirisasi berkonotasi kemajuan, ekstraksi berkonotasi pengurasan, dan keduanya menamai peristiwa yang sama. Istilah nikel hijau dipakai bagi pengolahan yang listriknya bertumpu pada batu bara, sehingga ia mendahului penilaian atas fakta."]
...
foot(s, "Keempat teori ini menyoroti dimensi yang luput dari lima teori sebelumnya: pembanding, relasi, keterwakilan, dan pilihan istilah.", 18);
...
s = pres.addSlide(); bg(s);
head(s, "11   SINTESIS", "Sintesis penilaian dan evaluasi klaim", "Tidak satu pun dari sembilan teori memberikan dukungan tanpa syarat.");
card(s, M, 1.94, 5.6, 4.88, SURF);
label(s, M + 0.34, 2.16, 5.0, "SEMBILAN PENILAIAN", EMBER);
[[1, "Egoism", "Netral", MUTED], [2, "Utilitarianism", "Ambivalen", GOLD], [3, "Ethics of duty", "Menolak", EMBER],
 [4, "Ethics of rights", "Bersyarat", GOLD], [5, "Justice", "Menolak", EMBER], [6, "Virtue ethics", "Menolak pembanding", EMBER],
 [7, "Ethic of care", "Menuntut pemulihan", GOLD], [8, "Discourse ethics", "Menolak prosedur", EMBER], [9, "Postmodern ethics", "Relativistik", MUTED]
...
s.addText("Enam menolak, satu bersyarat, satu ambivalen, dan satu netral karena mengakui keterbatasan teorinya sendiri.", {
  x: M + 0.56, y: 6.14, w: 4.51, h: 0.50, isTextBox: true, margin: 0, valign: "middle", fontFace: BF, fontSize: 10.5, color: PALE });
label(s, M + 6.05, 2.16, 6.0, "EMPAT PROPOSISI YANG DITURUNKAN DARI KESEMBILAN TEORI", EMBER);
[["Perbandingan masa lalu tidak bernilai normatif", "Kondisi yang lebih baik daripada ekspor bijih mentah tidak dengan sendirinya memadai. Virtue ethics menuntut pembanding pada standar tertinggi."],
...
/* ============ 20 SIMPULAN ============ */
s = pres.addSlide(); bg(s, true);
head(s, "PENUTUP", "Simpulan dan implikasi manajerial", "Tiga temuan dari pengujian, diikuti tiga tindak lanjut yang dapat dijalankan manajemen.", true);
[["Konvergensi, bukan kesepakatan", "Sembilan teori berangkat dari premis berbeda, namun mayoritasnya bermuara pada arah yang sama. Konvergensi dari titik masuk berbeda inilah bukti terkuat pluralisme."],
 ["Klaim etis yang gugur", "Yang gugur bukan kebijakan hilirisasinya, melainkan klaim bahwa kebijakan ini sudah etis. Upah, keselamatan, lingkungan, dan sumber energinya tetap terbuka diperbaiki."],
...
[["Internalisasi biaya tertunda", "Biaya kesehatan warga, pemulihan sungai, dan pensiun dini pembangkit masuk neraca sejak perencanaan, bukan diwariskan ke anggaran publik."],
 ["Pelibatan pihak yang absen", "Pekerja, warga hilir, dan masyarakat adat diberi keterwakilan formal dalam panitia keselamatan kerja, pemantauan lingkungan, dan perizinan."],
 ["Pengujian multiteori", "Klaim yang lolos utilitarianism kerap gugur pada keadilan distributif dan discourse ethics. Kesembilan teori berfungsi sebagai daftar periksa."]
].forEach((t, i) => { const xx = M + i * (cwa + 0.4);
