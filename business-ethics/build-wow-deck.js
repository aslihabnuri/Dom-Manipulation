...
function blob(s, x, y, w, h, fill) {
  s.addShape(pres.ShapeType.ellipse, { x, y, w, h, fill: { color: fill }, line: { color: fill, width: 0 } });
}
...
function rule(s, x, y, w, color) {
  rect(s, x, y, w, 0.022, color || RULEC);
}
function vrule(s, x, y, h, color) {
  rect(s, x, y, 0.022, h, color || RULEC);
}
function arrow(s, x, y, w, h, color, flipV) {
...
function lab(s, text, x, y, w, color) {
  rect(s, x, y + 0.055, 0.13, 0.13, color || ACC2);
...
TIM.forEach(function (m, i) {
  const yy = 4.48 + i * 0.40;
  s.addText([{ text: m[0] + "   ", options: { bold: true, color: WHITE } },
             { text: m[1], options: { color: "A5A3E0" } }],
    { x: M + 0.05, y: yy, w: 4.9, h: 0.30, isTextBox: true, margin: 0, valign: "middle", fontFace: BF, fontSize: 10.5 });
});
rule(s, M + 0.05, 5.78, 4.3);
...
burst(s, 0.22, 3.05, 3.95, 3.70, GREEN, { rotate: -6 });
img(s, "accent-megaphone", 0.60, 3.52, { w: 3.15 });
sticker(s, 0.58, 2.62, 3.15, 0.44, "BUKAN SEKADAR FIRASAT", { size: 10.5, rotate: -3 });
lab(s, "DEFINISI KONSEP", 4.70, 2.02, 3.5, PINK);
txt(s, "Teori etika normatif adalah aturan, pedoman, prinsip, dan pendekatan yang menentukan benar dan salah.",
  4.70, 2.34, 8.0, 0.80, { size: 17, bold: true, color: WHITE, face: HF, lh: 1.12 });
txt(s, "Crane dan Matten (2019: 87)", 4.70, 3.16, 7.9, 0.26, { size: 9.5, color: MUTE, italic: true, valign: "middle" });
rule(s, 4.70, 3.52, 8.06);
[["Rasionalisasi intuisi moral", ["Teori etika berfungsi ", ["merasionalkan, menjelaskan, dan memahami intuisi moral", "g"], " yang telah dimiliki mengenai benar dan salah."]],
 ...
 ["Justifikasi keputusan bisnis", ["Keputusan bisnis menuntut ", ["dasar rasional yang sistematis", "g"], " dan dapat dipertanggungjawabkan kepada pemangku kepentingan."]]
  ...
  const x = 4.70 + i * 2.80, y = 3.82 + i * 0.42;
  ...
  if (i === 0) carrow(s, x + 2.32, y + 0.20, 0.74, 0.64, { color: WHITE });
  if (i === 1) carrow(s, x + 2.34, y + 0.16, 0.74, 0.62, { color: WHITE, rotate: 8 });
});
...
[["KONSEKUENSIALIS", "Teleologis, menilai akibat tindakan", [["Egoism", 1], ["Utilitarianism", 2]], M],
 ["BERBASIS PRINSIP", "Deontologis, menilai kewajiban", [["Ethics of duty", 3], ["Ethics of rights", 4], ["Justice dan social contract", 5]], M + 3.65]
].forEach(function (g) {
  lab(s, g[0], g[3], 3.28, 3.3, PINK);
  txt(s, g[1], g[3], 3.56, 3.3, 0.26, { size: 9.5, italic: true, color: MUTE, valign: "middle" });
  g[2].forEach(function (t, i) {
    const yy = 3.94 + i * 0.38;
    tag(s, g[3], yy, 0.28, 0.28, String(t[1]), PINK, { size: 11, cs: 0 });
    txt(s, t[0], g[3] + 0.38, yy, 3.0, 0.28, { size: 11, bold: true, color: WHITE, valign: "middle" });
  });
});
rule(s, M, 5.42, 6.9);
...
[["Virtue ethics", 6, "Karakter pelaku"], ["Ethic of care", 7, "Relasi dan tanggung jawab"],
 ["Discourse ethics", 8, "Prosedur perumusan norma"], ["Postmodern ethics", 9, "Bahasa dan dorongan moral"]
  ...
  const yy = 3.48 + i * 0.72;
  tag(s, 9.15, yy, 0.32, 0.32, String(t[1]), GREEN, { size: 12, cs: 0, color: NEAR });
...
/* ---------- pola halaman teori: teks mengambang, kucing besar di satu sisi ---------- */
function theory(o) {
  ...
  if (o.layout === "third") {
    ...
    if (o.bubble) sticker(sl, 0.60, 4.52, 2.80, 0.50, o.bubble, { size: 10, rotate: -5 });
    star(sl, 1.98, 5.42, 0.68, 0.68, ACC, 24, -8);
    burst(sl, 3.45, 3.32, 3.30, 3.42, BC, { rotate: o.brot || 0 });
    const ih = 3.25, iw = sizeH(o.img, ih)[0];
    img(sl, o.img, 5.05 - iw / 2, 3.36, { h: ih });
    o.blocks.forEach(function (b, i) {
      const by = 3.40 + i * 1.06;
      lab(sl, b[0], 6.90, by, 5.4, ACC2);
      rich(sl, b[1], 6.90, by + 0.28, 5.88, 0.62, { fontSize: 10.5 });
      if (i < 2) rule(sl, 6.90, by + 0.94, 5.88);
    });
  } else {
    /* kolom kucing penuh di kiri (bawaan) atau kanan (cermin), teks mengambang di sisi lain */
    const flip = o.layout === "right";
    const IZ = flip ? 7.98 : M, TX = flip ? M : 5.75, TW = 6.9;
    ...
    rule(sl, TX, 3.30, TW);
    o.blocks.forEach(function (b, i) {
      const yy = 3.52 + i * 1.02;
      lab(sl, b[0], TX, yy, TW, ACC2);
      rich(sl, b[1], TX, yy + 0.28, TW, 0.66, { fontSize: 10.5 });
      if (i < 2) rule(sl, TX, yy + 0.92, TW);
    });
  }
...
theory({ no: 2, num: 7, img: "c-utilitarian", brot: -8, layout: "third",
  eyebrow: "04   TEORI MODERNIS BARAT   |   KONSEKUENSIALIS",
...
burst(s, 4.38, 1.86, 4.60, 4.45, BLUE, { rotate: 8 });
const b6w = 3.85;
...
/* ---------- pola dua teori alternatif: dua kolom mengambang plus kucing ---------- */
function altPair(o) {
  const sl = slide("blue");
  ...
  o.cards.forEach(function (c, i) {
    const x = o.cardX[i], cw = o.cw, g = i === 1;
    ...
    c.rows.forEach(function (r, j) {
      const yy = 3.80 + j * 0.92;
      lab(sl, r[0], x, yy, cw, AC1);
      rich(sl, r[1], x, yy + 0.27, cw, 0.60, { fontSize: 10 });
    });
  });
  src(sl, o.src); pageno(sl, o.num); sl.addNotes(o.notes);
}

altPair({ num: 12, cw: 4.00, cardX: [M, 8.78], eyebrow: "06   TEORI ALTERNATIF", title: "Virtue ethics dan ethic of care",
  ...
  cards: [
   ...
   { no: 7, name: "Ethic of care", q: "Solusi mana yang memelihara relasi dengan pihak terdampak?",
     ...
     rows: [["GAGASAN INTI", ["Manusia adalah ", ["makhluk yang saling bergantung", "g"], " dan terjalin dalam relasi, bukan individu rasional yang terpisah."]],
            ["TUNTUTAN NORMATIF", ["Empati, keselarasan, menghindari kerugian, dan ", ["tanggung jawab konkret pada orang tertentu", "g"], ", bukan aturan seragam."]],
...
altPair({ num: 13, cw: 3.95, cardX: [4.48, 8.83], eyebrow: "06   TEORI ALTERNATIF", title: "Discourse dan postmodern ethics",
...
burst(s, 4.80, 1.90, 3.75, 4.80, GREEN, { rotate: -8 });
img(s, "c-vc1", 4.89, 2.00, { h: 4.52 });
const qw = 4.30;
[[1, "Egoism", "Apakah semua pihak bebas mengejar kepentingan jangka panjangnya, dan sudahkah biaya tertunda dihitung?", PINK],
 ...
 [9, "Postmodern ethics", "Kepentingan siapa yang dilayani oleh istilah yang dipakai menamai kebijakan ini?", GREEN]
  ...
  const x = col === 0 ? M : 8.48, y = 1.96 + row * 0.94;
  txt(s, String(q[0]), x, y, 0.36, 0.30, { size: 14, bold: true, color: q[3], face: HF, valign: "middle" });
  txt(s, q[1], x + 0.42, y, qw - 0.42, 0.30, { size: 12, bold: true, color: WHITE, face: HF, valign: "middle" });
  txt(s, q[2], x + 0.42, y + 0.30, qw - 0.42, 0.42, { size: 9, color: BODY, lh: 1.1 });
  if (row < 4) rule(s, x, y + 0.78, qw);
});
rule(s, 8.48, 5.72, qw, PINK);
...
[["INSTRUMEN", "Permen ESDM Nomor 11 Tahun 2019 melarang ekspor bijih nikel, berlaku efektif 1 Januari 2020."],
 ...
 ["SENGKETA", "Panel WTO memutus kebijakan ini melanggar aturan perdagangan pada November 2022."]
  ...
  const x = M + i * 2.50;
  bignum(s, (i + 1) + ".", x, 1.90, PINK, 24);
  tag(s, x, 2.34, 1.72, 0.28, a[0], PINK, { size: 8.5, cs: 1.0 });
  txt(s, a[1], x, 2.74, 2.27, 1.15, { size: 10, color: BODY, lh: 1.16 });
});
lab(s, "KRONOLOGI", M, 4.10, 3.0, MUTE);
rule(s, M, 4.86, 7.05, "5B58C4");
[["2014", "Larangan pertama, lalu direlaksasi"], ["2019", "Permen ESDM Nomor 11 terbit"],
 ["2020", "Larangan berlaku efektif"], ["2022", "Panel WTO memenangkan Uni Eropa"], ["2025", "Ekspor olahan 40 miliar dolar"]
  ...
  const x = M + 0.10 + i * 1.44;
  blob(s, x, 4.79, 0.16, 0.16, GREEN);
  txt(s, k[0], x - 0.12, 4.42, 1.1, 0.32, { size: 15, bold: true, color: GREEN, face: HF, valign: "middle" });
  txt(s, k[1], x - 0.12, 5.02, 1.34, 0.55, { size: 7.5, color: BODY, lh: 1.1 });
});
rule(s, M, 5.78, 7.05, PINK);
rich(s, ["Manfaatnya besar, terukur, dan terkonsentrasi, sedangkan bebannya tersebar, tertunda, dan sulit dikuantifikasi. Persoalan dengan struktur demikian ",
  ["tidak dapat diselesaikan melalui kriteria penilaian tunggal", "a"], "."],
  M, 5.92, 7.05, 0.62, { fontSize: 10 });
...
const nw = (CW - 0.35) / 2;
tag(s, M, 2.00, 3.55, 0.34, "PENDUKUNG   HILIRISASI BERDAULAT", GREEN, { size: 9.5, color: NEAR });
tag(s, M + nw + 0.35, 2.00, 3.85, 0.34, "PENENTANG   EKSTRAKSI BERBIAYA SOSIAL", PINK, { size: 9.5 });
vrule(s, W / 2 - 0.01, 2.10, 4.30);
[[M, ["40 miliar dolar", "41,5 miliar dolar", "166 ribu pekerja", "Pemasok terbesar"],
  ["Nilai ekspor produk nikel olahan pada 2025, naik dari 3 miliar dolar pada 2020",
   ...
   "Indonesia menjadi simpul utama rantai pasok baterai dunia"], "c-narr1", PINK, GREEN],
 [M + nw + 0.35, ["3 sampai 3,6 juta", "107 pekerja meninggal", "163 ribu hektare", "4,35 persen"],
  ["Upah pokok per bulan, di bawah upah minimum Morowali 2025 sebesar 3,7 juta",
   ...
   "Nilai tambah yang tinggal di Morowali, kemiskinannya 12,58 persen"], "c-narr2", GREEN, PINK]
].forEach(function (c) {
  const cx = c[0];
  burst(s, cx + 3.22, 3.42, 2.80, 3.00, c[4], { rotate: 10 });
  img(s, c[3], cx + 3.35, 3.60, { w: 2.40 });
...
[[1, "Egoism", "NETRAL", "5B58C4", ["Pada horizon pendek jelas melayani kepentingan Indonesia. Pada horizon panjang, biaya kesehatan warga, pemulihan sungai, dan pensiun dini pembangkit batu bara belum diinternalisasi. ", ["Kepentingan diri yang dihitung cermat justru menuntut biaya tertunda itu masuk sejak awal", "g"], "."]],
 ...
 [5, "Justice", "MENOLAK", PINK, ["Manfaat mengalir ke penerimaan nasional dan pemodal, sedangkan beban terkonsentrasi pada pekerja kawasan, warga hilir sungai, dan masyarakat adat. ", ["Kabupaten penghasil justru lebih miskin daripada rata-rata provinsinya", "g"], " dan hanya 4,35 persen nilai tambah tinggal di daerah."]]
  ...
  const y = 1.94 + i * 0.96;
  txt(s, String(l[0]), RX17, y + 0.02, 0.4, 0.32, { size: 15, bold: true, color: l[3], face: HF, valign: "middle" });
  txt(s, l[1], RX17 + 0.42, y + 0.02, 1.95, 0.28, { size: 12, bold: true, color: WHITE, face: HF, valign: "middle" });
  tag(s, RX17 + 0.42, y + 0.40, 1.60, 0.28, l[2], l[3], { size: 8 });
  rich(s, l[4], RX17 + 2.45, y, rw - 2.45, 0.78, { fontSize: 10, lineSpacingMultiple: 1.12 });
  if (i < 4) rule(s, RX17, y + 0.86, rw);
});
...
burst(s, 4.96, 1.92, 3.45, 4.70, GREEN, { rotate: -8 });
img(s, "c2-synth", 5.19, 2.02, { h: 4.38 });
tag(s, M, 2.02, 2.45, 0.30, "SEMBILAN PENILAIAN", PINK, { size: 9.5 });
[[1, "Egoism", "Netral", "9C9AD8"], [2, "Utilitarianism", "Ambivalen", "F0C24A"], [3, "Ethics of duty", "Menolak", PINK],
 ...
 [7, "Ethic of care", "Menuntut pemulihan", "F0C24A"], [8, "Discourse ethics", "Menolak prosedur", PINK], [9, "Postmodern ethics", "Relativistik", "9C9AD8"]
  ...
  const y = 2.52 + i * 0.37;
  txt(s, String(m[0]), M, y, 0.30, 0.30, { size: 11, bold: true, color: i < 5 ? PINK : GREEN, face: HF, valign: "middle" });
  txt(s, m[1], M + 0.36, y, 1.85, 0.30, { size: 11, bold: true, color: WHITE, valign: "middle" });
  txt(s, m[2], M + 2.28, y, 2.02, 0.30, { size: 10.5, bold: true, color: m[3], align: "right", valign: "middle" });
});
rule(s, M, 5.92, 4.30, PINK);
txt(s, "Enam menolak, satu bersyarat, satu ambivalen, dan satu netral karena mengakui keterbatasan teorinya.",
  M, 6.04, 4.30, 0.48, { size: 9, color: BODY, lh: 1.1 });
tag(s, 8.48, 2.02, 4.30, 0.30, "EMPAT PROPOSISI DARI KESEMBILAN TEORI", GREEN, { size: 9.5, color: NEAR });
[["Perbandingan masa lalu tidak normatif", ["Kondisi yang lebih baik daripada ekspor bijih mentah tidak dengan sendirinya memadai. ", ["Virtue ethics menuntut pembanding pada standar tertinggi", "g"], "."]],
 ...
 ["Prosesnya cacat sejak perumusan", ["Warga terdampak tidak menjadi pihak dalam perizinan dan perencanaan kawasan. Discourse ethics membatalkan hasil dari proses semacam itu."]]
  ...
  const y = 2.52 + i * 1.04;
  txt(s, String(i + 1), 8.48, y, 0.34, 0.28, { size: 14, bold: true, color: GREEN, face: HF, valign: "middle" });
  txt(s, p[0].toUpperCase(), 8.90, y, 3.88, 0.28, { size: 11, bold: true, color: WHITE, face: HF, cs: -0.2, valign: "middle" });
  rich(s, p[1], 8.90, y + 0.30, 3.88, 0.58, { fontSize: 9.5, lineSpacingMultiple: 1.1 });
  if (i < 3) rule(s, 8.48, y + 0.92, 4.30);
});
...
[["Konvergensi, bukan kesepakatan", ["Sembilan teori berangkat dari premis berbeda, namun mayoritasnya bermuara pada arah yang sama. ", ["Inilah bukti terkuat pluralisme", "fw"], "."]],
 ...
 ["Kasus yang masih berlangsung", ["Kebijakannya masih berjalan, sengketa dagangnya belum tuntas, dan angkanya berubah tiap tahun. Kasus yang belum selesai justru paling layak didiskusikan."]]
  ...
  const x = M + i * 2.50;
  bignum(s, (i + 1) + ".", x, 1.90, PINK, 24);
  txt(s, t[0].toUpperCase(), x, 2.38, 2.27, 0.54, { size: 11.5, bold: true, color: WHITE, face: HF, lh: 1.02, cs: -0.2 });
  rich(s, t[1], x, 2.96, 2.27, 1.15, { fontSize: 9 });
});
rule(s, M, 4.28, 7.27, GREEN_D);
lab(s, "IMPLIKASI MANAJERIAL", M, 4.44, 4.0, ACC2);
[["Internalisasi biaya tertunda", "Biaya kesehatan warga, pemulihan sungai, dan pensiun dini pembangkit masuk neraca sejak perencanaan."],
 ...
 ["Pengujian multiteori", "Klaim yang lolos utilitarianism kerap gugur pada keadilan distributif dan discourse ethics."]
  ...
  const x = M + i * 2.50;
  txt(s, t[0].toUpperCase(), x, 4.84, 2.27, 0.44, { size: 10, bold: true, color: "9BE8AB", face: HF, cs: -0.2, lh: 1.0 });
  txt(s, t[1], x, 5.30, 2.27, 0.80, { size: 8.5, color: BODY, lh: 1.14 });
});
rule(s, M, 6.24, 7.27, PINK);
s.addText(runs([["Manfaat yang terukur selalu lebih mudah dipertahankan daripada beban yang tersebar. ", "fw"],
  ["Justru karena itu beban yang tersebar perlu dihitung lebih dahulu.", "a"]], { fontSize: 10.5 }),
  { x: M, y: 6.38, w: 7.27, h: 0.55, isTextBox: true, margin: 0, valign: "top", fontFace: BF, fontSize: 10.5, lineSpacingMultiple: 1.12 });
pageno(s, 20);
