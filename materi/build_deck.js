...
// skala tipe
const TITLE = 44, SUB = 28, BODY = 16, CAP = 13;
...
function numList(s, items, x, y, w, opts = {}) {
  ...
  items.forEach((it, i) => {
    ...
    s.addText(String(i + 1), { x, y: yy, w: 0.4, h: rowH, fontFace: BF, fontSize: 16, color: GRAY, isTextBox: true, margin: 0, valign: "middle" });
...
// ============================================================ 1. SAMPUL
{
  ...
  s.addText("KELOMPOK 4", { x: FX + 0.08, y: ribbonY, w: 1.6, h: ribbonH, fontFace: BF, fontSize: 14, bold: true, color: BLACK, align: "center", valign: "middle", isTextBox: true, margin: 0 });
  s.addText("Bagaskoro  ·  25/574280/PEK/31778          Aulia Sisca Rahmadiyanti  ·  25/574305/PEK/31789          Fitra Aidila  ·  25/574309/PEK/31791",
    { x: FX + 1.95, y: ribbonY, w: FW - 2.2, h: ribbonH, fontFace: BF, fontSize: 13, color: GRAY, valign: "middle", isTextBox: true, margin: 0 });
...
// ============================================================ 2. TUJUAN & ALUR
{
  ...
  const items = ["Kerangka berpikir: strategi dulu, baru teknis", "Kisah sukses: dari pengecer virtual ke pengelola persediaan", "Proses pemenuhan pesanan: delapan langkah", "Analisis trade-off: nyata atau searah", "Kaitan dengan teori persediaan", "Perkembangan terbaru: 2023 dan 2025", "Lesson learned: apa yang bisa ditiru"];
  items.forEach((it, i) => {
    const col = i < 4 ? 0 : 1, row = i < 4 ? i : i - 4;
    const x = TX + 1.25 + col * 5.35, y = TOP + 2.85 + row * 0.66;
    s.addText(String(i + 1), { x, y, w: 0.4, h: 0.52, fontFace: BF, fontSize: 16, color: GRAY, isTextBox: true, margin: 0, valign: "middle" });
    HL(s, it, x + 0.4, y, 4.7, 0.52, { size: 15 });
  });
}
...
{
  const s = frame();
  const im = cut(s, "conveyor", { h: 3.2, maxW: 5.6 }); const W = 6.0;
...
// ============================================================ 6. TRANSFORMASI (//01 //02)
{
  ...
  HL(s, "RENCANA 1995: PENGECER VIRTUAL", FX + 1.25, FY + 2.1, 5.7, 0.42, { size: 19, bold: true });
  HL(s, "Tanpa persediaan, tanpa gudang, tanpa overhead: komputer menerima pesanan buku dan meneruskan pemenuhannya ke pihak lain.", FX + 1.25, FY + 2.6, 5.7, 0.78, { fill: WHITE, color: GRAY, size: 14 });
  label(s, "//02", FX + 6.2, FY + 3.55, 58, { color: MAGENTA });
  HL(s, "KENYATAAN: GUDANG TERBESAR DI DUNIA", FX + 6.85, FY + 4.4, 5.5, 0.42, { size: 19, bold: true });
  HL(s, "150+ gudang, 200 juta item di situs, 200.000 pieces per hari dari satu gudang, tenaga kerja kurang dari 3 menit per pesanan.", FX + 6.85, FY + 4.9, 5.5, 0.78, { fill: WHITE, color: GRAY, size: 14 });
}
...
// ============================================================ 7. APAKAH RENCANA AWALNYA SALAH
{
  ...
  numList(s, ["Lowest price: harga terendah", "Fastest delivery: pengiriman tercepat", "Error-free: pemenuhan pesanan bebas kesalahan"], TX + 1.15, TOP + 2.65, 5.0, { rowH: 0.42, gap: 0.1 });
...
// ============================================================ 8. 4V
{
  ...
  let y = TOP + 0.9;
  y = block(s, "VOLUME · SANGAT TINGGI", "200.000 pieces per hari per gudang; otomasi menjadi ekonomis.", TX, y, W, { pt: 26, h: 0.3 }) + 0.12;
  y = block(s, "VARIETY · SANGAT TINGGI", "200 juta item; akurasi catatan per item menjadi persoalan utama.", TX, y, W, { pt: 26, h: 0.3 }) + 0.12;
  y = block(s, "VARIATION · TINGGI", "Musim puncak; gudang dirancang untuk kapasitas maksimum.", TX, y, W, { pt: 26, h: 0.3 }) + 0.12;
  y = block(s, "VISIBILITY · RENDAH", "Pelanggan hanya melihat harga, waktu tiba, dan isi paket.", TX, y, W, { pt: 26, h: 0.3 }) + 0.15;
...
function stepsSlide(from, img, title) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    HL(s, STEPS[from + i][0], x + 0.55, y + 0.72, 5.35, 0.42, { size: 17, bold: true });
    HL(s, STEPS[from + i][1], x + 0.55, y + 1.24, 5.35, 0.78, { fill: WHITE, color: GRAY, size: 14 });
  }
}
...
// ============================================================ 12. ROBOT KIVA
{
  ...
  let y = numList(s, ["10.000+ robot membawa rak ke pekerja: barangnya yang bergerak", "20 mil jalan kaki per hari per pekerja dihemat", "Pick and scan naik dari 100 menjadi 300 item per jam", "Biaya per pesanan turun 20–40%; US$400–900 juta per tahun"], TX, TOP + 1.3, W, { rowH: 0.48, gap: 0.1, size: 15 });
  y += 0.3;
...
// ============================================================ 13. PRODUKTIVITAS
{
  ...
  T(s, "100 menjadi 300 item per jam mudah dibaca sebagai kenaikan 200%. Itu single-factor: biaya robot tidak ikut dihitung.", TX, TOP + 0.85, W, 0.62);
  let y = TOP + 1.65;
  y = block(s, "SINGLE-FACTOR · +200%", "Benar, tetapi belum memperhitungkan investasi robot.", TX, y, W, { pt: 26, h: 0.3 }) + 0.12;
  y = block(s, "MULTI-FACTOR · BIAYA PER PESANAN −20–40%", "Sudah termasuk biaya robot: hemat US$0,70–1,50 per pesanan.", TX, y, W, { pt: 26, h: 0.3 }) + 0.12;
...
{
  const s = frame();
  const im = cut(s, "truck", { h: 2.6, maxW: 5.4 }); const W = 6.4;
  ...
  const rows = [
    ...
    ["FLEXIBILITY", "70% pesanan multiproduk dilayani satu sistem batch picking."],
    ["COST", "Harga terendah; 60% lewat USPS; biaya proses turun 20–40%."],
  ];
  let y = TOP + 0.85;
  rows.forEach((r, i) => { y = block(s, r[0], r[1], TX, y, i < 3 ? W : 5.6, { pt: 22, h: 0.3, size: 15 }) + 0.1; });
  HL(s, "Satu sistem yang sama: stok yang akurat lokasinya, proses pendek, kontrol berlapis.", TX, y, 5.6, 0.62);
}
...
// ============================================================ 16. TIGA TRADE-OFF
{
  ...
  const tos = [
    ["Holding cost vs service level", "Carrying cost sekitar 26% per tahun. Aturan: naikkan stok kalau biaya kekosongan lebih besar dari biaya simpan; untuk ritel daring, terpenuhi."],
    ["Otomasi vs kesalahan dan tenaga kerja", "Aturan: otomasi layak kalau penghematan per pesanan dikali volume tahunan lebih besar dari biaya otomasi. Penentunya volume."],
    ["Kecepatan kirim vs biaya kirim", "60% pesanan lewat USPS yang murah. Aturan: kecepatan dibangun dari dalam, stok dekat pelanggan dan proses pendek."],
  ];
  let y = TOP + 1.65;
  tos.forEach((t, i) => {
    s.addText(String(i + 1), { x: TX, y, w: 0.4, h: 0.42, fontFace: BF, fontSize: 16, color: GRAY, isTextBox: true, margin: 0, valign: "middle" });
    HL(s, t[0], TX + 0.4, y, W - 0.4, 0.42, { bold: true });
    T(s, t[1], TX + 0.4, y + 0.5, W - 0.4, 0.62, { size: 15 });
    y += 1.3;
  });
}
...
{
  const s = frame();
  const im = cut(s, "conveyor", { h: 3.0, maxW: 5.3 }); const W = 6.0;
  ...
  let y = numList(s, ["Akurasi vs biaya: searah, mencegah biaya return", "Kecepatan proses vs biaya: searah, kurang dari 3 menit", "Ketersediaan stok vs holding cost: trade-off nyata, dipilih sadar", "Kecepatan kirim vs biaya kirim: trade-off nyata yang dihindari"], TX, TOP + 1.65, W, { rowH: 0.48, gap: 0.1, size: 15 });
  y += 0.2;
  doodle(s, TX, y, 0.9);
  label(s, "EFFICIENT FRONTIER", TX + 1.1, y + 0.25, 32);
  T(s, "Pengecer virtual gagal memenuhi constraint; gudang manual didominasi otomasi pada volume Amazon; gudang terotomasi berada di frontier. Pada volume kecil, alternatif manual kembali ke frontier.", TX + 1.1, y + 0.9, 5.2, 0.9, { size: 15 });
}
...
// ============================================================ 18. KAITAN TEORI
{
  ...
  let y = numList(s, [
    ...
    "Penempatan di gudang: picking diurutkan, slotting oleh perangkat lunak (eksplisit)",
  ], TX, TOP + 1.3, W, { rowH: 0.42, gap: 0.08, size: 14 });
...
// ============================================================ 19. PERKEMBANGAN TERBARU
{
  ...
  HL(s, "Keputusan where kini per wilayah; 100.000 item terlaris adalah Pareto/ABC.", TX, y, W, 0.42); y += 0.62;
...
// ============================================================ 20. LESSON LEARNED (//01 ... //06)
{
  ...
  L.forEach((l, i) => {
    ...
    HL(s, l[0], x + 0.5, y + 0.62, 5.45, 0.36, { size: 16, bold: true });
    HL(s, l[1], x + 0.5, y + 1.16, 5.45, 0.32, { fill: WHITE, color: GRAY, size: 14 });
  });
}
