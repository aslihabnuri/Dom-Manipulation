...
## Layout (editorial grid, diagonal balance)
- 72 px margins, two vertical axes: everything on the left aligns to the left margin, everything on the right aligns to the right margin.
- **Masthead row**: logo (left) and `EVERYDAY ESSENTIALS` (right) on one baseline, a hairline rule beneath spanning margin to margin.
- **Top-right**: `Free shipping` / `Extra IDR 5K voucher for new buyers`, right-aligned under the rule, in the wall space above the model's head.
- **Hero**: `SAVE UP TO` sized so its width equals the display number (a justified lockup, not over-tracked), then `25%` in Zalando Sans Expanded Black spanning about 72% of the width. The model is cut out locally (rembg, no kie.ai credits) and layered in front, so the number runs behind the head, magazine-cover style, which fills the wall area without crowding the face.
- `Made to move with you.` (brand-guideline brand-story line) under the number in Davi's Grey.
- **Bottom-right (sofa)**: `SHOP NOW` pill and `*Terms & conditions apply`, right-aligned. The legs at bottom-left stay uncovered.
- Type in Dark Charcoal `#282828` on the light scene; no gradients or overlays.
...
3. `scripts/compose.py PHOTO OUT [scale] [percent] [bigw] [hero_y] [cutout]` sets the typography with the brand fonts. Final: `scale 2, 25%, 780, 0.152, cutout.png`.
