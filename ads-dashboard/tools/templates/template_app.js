...
async function gdSearchChildren(server,parentId){
  try{
    return await window.claude.mcp.callTool(server,'search_files',
      {query:`parentId = '${parentId}'`,pageSize:200,excludeContentSnippets:true},{cache:false});
  }catch(e){
    if(e&&e.retryable){
...
/* kumpulkan file secara rekursif: folder channel + subfolder di dalamnya
   (mis. tim membuat subfolder "JULY 2026" di tiap folder channel) */
async function gdGatherFiles(server,rootId,onStatus,label){
  const files=[]; let queue=[rootId], depth=0, guard=0;
  while(queue.length&&depth<5&&guard<80){
    const next=[];
    for(const fid of queue){
      guard++;
      const res=await gdSearchChildren(server,fid);
      const items=(res&&res.payload&&res.payload.files)||[];
      for(const it of items){
        if(it.mimeType==='application/vnd.google-apps.folder'){next.push(it.id);onStatus(L('Memindai ','Scanning ')+label+' / '+(it.title||'')+'…');}
        else files.push(it);
      }
    }
    queue=next; depth++;
...
async function runDriveSync(onStatus){
  ...
  for(const f of GD_FOLDERS){
    ...
    for(const file of list){
      ...
      /* connector artifact tidak sanggup menarik file besar (>5MB) → 502.
         Lewati dengan pesan jelas; file besar diproses lewat unggah manual atau versi online. */
      const fsize=+(file.fileSize||0);
      if(fsize>5*1024*1024){
        report.skipped.push(t+': '+L(`terlalu besar (${(fsize/1048576).toFixed(1)}MB) untuk sinkron di browser. Unggah manual file ini di kotak atas, atau pakai versi online.`,`too large (${(fsize/1048576).toFixed(1)}MB) for in-browser sync. Upload this file manually in the box above, or use the online version.`));
        continue;
      }
      ...
      try{dl=await window.claude.mcp.callTool(server,'download_file_content',{fileId:file.id},{cache:false});}
        ...
        if(e&&e.retryable){
          await new Promise(r=>setTimeout(r,(e.retryAfterMs||1500)+Math.random()*500));
          try{dl=await window.claude.mcp.callTool(server,'download_file_content',{fileId:file.id},{cache:false});}
          catch(e2){report.errors.push(t+': '+gdErrMsg(e2)+(String(e2&&e2.code).includes('unavailable')||/502/.test(gdErrMsg(e2))?L(' (mungkin file terlalu besar; unggah manual)',' (possibly too large; upload manually)'):''));continue;}
        } else {report.errors.push(t+': '+gdErrMsg(e));continue;}
      }
...
function renderUpload(){
  ...
  <div class="card" style="margin-top:12px">
    ...
    ${gdAvailable()?`
    <p class="note" style="margin:0 0 10px">${L('Tarik raw data terbaru dari folder "SM Ads Raw Data" di Google Drive (termasuk subfolder di dalamnya). File yang sudah diproses dilewati otomatis. Data masuk sebagai overlay di browser ini; untuk membagikannya ke semua pemegang link, minta Claude menerbitkan ulang. Catatan: file sangat besar (di atas 5MB, mis. RAW TikTok Product) tidak bisa ditarik lewat sinkron browser, unggah manual file itu di kotak atas.','Pull the latest raw data from the "SM Ads Raw Data" folder in Google Drive (including its subfolders). Files already processed are skipped automatically. Data lands as an overlay in this browser; to share it with all link holders, ask Claude to republish. Note: very large files (over 5MB, e.g. RAW TikTok Product) cannot be pulled via in-browser sync, upload those manually in the box above.')}</p>
