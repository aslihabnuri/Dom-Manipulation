...
const CH_LABEL={
  tt_live_smo:['Live GMV Max SMO','Live GMV Max SMO'],
  tt_live_sid:['Live GMV Max SID','Live GMV Max SID'],
  tt_live_smv:['Live GMV Max SMV','Live GMV Max SMV'],
  tt_live_aff:['Live GMV Max Affiliate','Live GMV Max Affiliate'],
  tt_live_all:['Live GMV Max (agregat)','Live GMV Max (aggregate)'],
  tt_vsa:['Video Shopping Ads','Video Shopping Ads'],
  tt_vsa_ns:['Video Shopping Ads NS','Video Shopping Ads NS'],
  tt_psa:['Product Card Shopping Ads','Product Card Shopping Ads'],
  tt_psa_ns:['Product Card Shopping Ads NS','Product Card Shopping Ads NS'],
  tt_vsa_psa:['VSA + PSA (agregat)','VSA + PSA (aggregate)'],
  tt_ci_smo:['Community Interaction SMO','Community Interaction SMO'],
  tt_ci_smv:['Community Interaction SMV','Community Interaction SMV'],
  ...
  sp_live:['Iklan Live Shopee (LSA)','Shopee Live Ads (LSA)'],
  sp_sba:['Search Brand Ads (Banner)','Search Brand Ads (Banner)'],
...
function dailyOptions(includeAgg=true){
  ...
  for(const p in PLATFORMS){
    ...
    h+=`<optgroup label="${PLATFORMS[p].name}">${list.map(c=>`<option value="${c.id}">${esc(chName(c.id))}${c.objective!=='sales'?' · upper funnel':''}</option>`).join('')}</optgroup>`;
  }
  return h;
}
...
function momSectionHtml(y){
  ...
  <h2 class="sec">${L('Tabel Month over Month','Month over Month Table')}
    <span class="hint">${L('angka resmi dari sheet MoM ','official figures from the MoM sheet ')}${y}</span>
...
function renderOverview(){
  ...
  view.innerHTML=`
  <h2 class="sec">${L('Ringkasan Semua Platform','All Platform Summary')} <span class="hint">${esc(curLabel)} · ${L('pembanding','compared with')}: ${esc(cmpLabel)}${momNote}</span></h2>
  <div class="controls">
  ...
  <div class="grid g-kpi">
    ${kpiTile(OV.basis==='all'?L('Total Spend (Semua Iklan)','Total Spend (All Ads)'):L('Total Spend (Iklan Penjualan)','Total Spend (Sales Ads)'),fmtRpC(st.spend),
      ...
      (OV.basis==='all'&&st.spendUpper?`<span class="delta" style="display:block">${L('penjualan','sales')} ${fmtRpC(st.spendSales)} · upper ${fmtRpC(st.spendUpper)}</span>`:''))}
  ...
  <div class="grid" style="grid-template-columns:1.6fr 1fr;margin-top:12px" id="yearGrid">
    <div>
      <div class="card" style="margin-bottom:12px"><h3>${L('GMV per Bulan per Platform','Monthly GMV per Platform')} <span class="hint">· Rp</span></h3><div id="ovGmv"></div></div>
      <div class="card"><h3>${L('Spend per Bulan per Platform','Monthly Spend per Platform')} <span class="hint">· Rp</span></h3><div id="ovCost"></div></div>
    </div>
    <div>${yearSideHtml(OV.year)}</div>
  </div>
  <style>@media(max-width:960px){#yearGrid{grid-template-columns:1fr}}</style>
  `:`
  <div class="card" style="margin-top:12px"><h3>${L('Tren Metrik Harian (channel penjualan)','Daily Metric Trend (sales channels)')} <span class="hint">· ${L('centang metrik yang ingin ditampilkan','tick the metrics to display')}</span></h3><div id="ovTrend"></div></div>
  <div class="grid g-2" style="margin-top:12px">
    <div class="card"><h3>${L('GMV Harian per Platform','Daily GMV per Platform')} <span class="hint">· Rp</span></h3><div id="ovGmv"></div></div>
    <div class="card"><h3>${L('Spend Harian per Platform','Daily Spend per Platform')} <span class="hint">· Rp</span></h3><div id="ovCost"></div></div>
  </div>`}
  <div class="grid g-2" style="margin-top:12px">
  ...
  <h2 class="sec">${L('Ringkasan per Platform','Summary per Platform')} <span class="hint">${esc(curLabel)} · spend ${OV.basis==='all'?L('termasuk upper funnel','including upper funnel'):L('penjualan saja','sales only')}</span></h2>
...
function renderDaily(){
  view.innerHTML=`
  <h2 class="sec">${L('Database Harian','Daily Database')} <span class="hint">${L('data harian per channel, 2025 hingga 2026','daily data per channel, 2025 through 2026')}</span></h2>
  <div class="controls">
...
function renderDailyChannel(root){
  ...
  <div class="grid g-kpi">
    ...
    ${S.sales?kpiTile('GMV',fmtRpC(st.gmv),'',spark(gmv,'var(--good)')):''}
    ${S.sales?kpiTile('ROAS',fmtX(st.roas)):''}
    ${S.sales?kpiTile(L('Pesanan','Orders'),fmtN(st.orders)):''}
    ${S.sales?kpiTile('CPO',fmtRpC(st.cpo)):''}
    ${S.sales?kpiTile('AOV',fmtRpC(st.aov)):''}
  </div>
  <div class="card" style="margin-top:12px"><h3>${L('Tren Harian','Daily Trend')} · ${esc(S.name)} <span class="hint">· ${L('centang metrik yang ingin ditampilkan','tick the metrics to display')}</span></h3><div id="dTrend"></div></div>
  <h2 class="sec">${L('Tabel Harian','Daily Table')} · ${esc(S.name)} <span class="hint">${isoLabel(from)} - ${isoLabelY(to)}</span></h2>
...
function renderCompare(){
  ...
  view.innerHTML=`
  <h2 class="sec">${L('Bandingkan','Compare')} <span class="hint">${L('periode vs periode, atau channel vs channel, bebas lintas bulan dan tahun','period vs period, or channel vs channel, freely across months and years')}</span></h2>
  <div class="controls">
  ...
  function drawCompare(){
    ...
    <div class="grid g-2">
      ...
      <div class="card"><h3>${L('GMV Harian, A vs B','Daily GMV, A vs B')} ${isPeriode?`<span class="hint">· ${L('disejajarkan per hari ke-n','aligned by day number')}</span>`:''}</h3><div id="cChart"></div>
...
function renderPlatforms(){
  ...
  view.innerHTML=`<h2 class="sec">${L('Breakdown per Platform','Breakdown per Platform')} <span class="hint">${L('klik baris untuk detail harian · klik angka anggaran untuk mengubahnya','click a row for daily detail · click a budget figure to edit it')}</span></h2>
  <div class="controls">
    <label>${L('Bulan','Month')}:</label><select id="platMonth">${monthOptions(mk)}</select>
  ...
  <div id="platWrap"></div>`;
  document.getElementById('platMonth').onchange=e=>{platMonth=e.target.value;renderPlatforms();};
  ...
  for(const p in PLATFORMS){
    ...
    <div class="card" style="margin-bottom:14px">
      <div style="display:flex;flex-wrap:wrap;gap:18px;align-items:baseline;margin-bottom:10px">
        ...
        ${upCost?`<span style="font-size:12.5px;color:var(--ink-2)">Spend Upper <b>${fmtRpC(upCost)}</b></span>`:''}
      ...
      ${uppers.length?`<div style="font-size:11.5px;font-weight:700;letter-spacing:.04em;color:var(--muted);margin:12px 0 2px">UPPER FUNNEL <span style="font-weight:400;letter-spacing:0">· Awareness / Brand Consideration / Community Interaction</span></div>
      <div class="tbl-wrap" style="border:0;border-radius:0"><table><thead><tr>
      ...
      <p class="note" style="margin-top:4px">${L('*Klik Produk pada Brand Consideration adalah Consideration Size sesuai definisi database Anda. Biaya upper funnel ikut dihitung pada Total Spend (Semua Iklan) di tab Ringkasan.','*Product Clicks on Brand Consideration equals Consideration Size per your database definition. Upper funnel spend is included in Total Spend (All Ads) on the Summary tab.')}</p>`:''}
    </div>`;
  ...
  wrap.querySelectorAll('tr[data-ch]').forEach(tr=>tr.addEventListener('click',()=>{
    dailySel=tr.dataset.ch;dailyMode='channel';DH={from:isoOf(mk,1),to:isoOf(mk,mkDays(mk)),preset:'custom'};
...
function renderAnalytics(){
  ...
  view.innerHTML=`
  <h2 class="sec">${L('Analisis dan Peringatan','Analysis and Warnings')} <span class="hint">${mkLabel(LATEST)} · ${L('otomatis dari data harian, produk, dan kreator','automatically from daily, product, and creator data')}</span></h2>
  <div class="grid g-3">
  ...
  <h2 class="sec">${L('Analisis Produk','Product Analysis')} <span class="hint">${PI?mkLabel(PI.mk)+' · '+PI.n+L(' produk',' products'):''}</span></h2>
  ...
  <div class="grid g-2">
    <div class="card"><h3>${L('Produk penggerak nilai','Value driving products')} <span class="hint">· ${L('kontribusi terhadap total nilai','contribution to total value')}</span></h3>
  ...
  <h2 class="sec">${L('Analisis Kreator','Creator Analysis')} <span class="hint">${KI?mkLabel(KI.mk)+' · '+KI.n+L(' kreator',' creators'):''}</span></h2>
  ...
  <h2 class="sec">${L('Target ROAS per Channel','ROAS Target per Channel')} <span class="hint">${L('dipakai mesin peringatan · tersimpan di browser','used by the warning engine · saved in the browser')}</span></h2>
...
function renderUpload(){
  ...
  const dmin=FIRST_ISO,dmax=isoOf(LATEST,mkDays(LATEST));
  view.innerHTML=`
  <h2 class="sec">${L('Unggah Data','Upload Data')} <span class="hint">${L('letakkan file ekspor platform di sini, dashboard yang memproses','drop platform export files here, the dashboard processes them')}</span></h2>
  <div class="card">
  ...
  <div class="grid g-2" style="margin-top:12px">
    <div class="card">
      <h3>${L('Input Manual','Manual Input')} <span class="hint">· ${L('nilai boleh berbeda per tanggal','values may differ per date')}</span></h3>
    ...
    <div class="card">
      <h3>${L('Overlay aktif','Active overlay')} <span class="hint">· ${ovEntries.length} ${L('sel','cells')}</span></h3>
  ...
  <div class="card" style="margin-top:12px">
    <h3>${L('Sumber Data per Bulan','Data Source per Month')} <span class="hint">· ${L('hindari tumpang tindih sheet dan unggahan','avoid overlap between sheet and uploads')}</span></h3>
    ...
    <div style="margin-top:14px;border-top:1px solid var(--grid);padding-top:12px">
      <h3 style="margin-bottom:6px">${L('GMV Marketplace Bulanan','Monthly Marketplace GMV')} <span class="hint">· ${L('untuk baris Total GMV All Marketplace pada tabel MoM, diisi manual di sini','for the Total GMV All Marketplace row in the MoM table, filled manually here')}</span></h3>
      <div class="controls" style="margin:0">
...
function renderProducts(){
  ...
  view.innerHTML=`
  <h2 class="sec">${L('Produk','Products')} <span class="hint">${L('quantity sold dan nilai · TikTok dan Shopee, tanpa Live Ads','quantity sold and value · TikTok and Shopee, excluding Live Ads')}</span></h2>
  <div class="controls">
  ...
  <h2 class="sec">${L('Daftar Produk','Product List')} <span class="hint">${list.length} ${L('produk · klik judul kolom untuk mengurutkan','products · click a column header to sort')}</span></h2>
...
function renderCreators(){
  ...
  view.innerHTML=`
  <h2 class="sec">${L('Kreator','Creators')} <span class="hint">${L('performa kreator dan video dari TikTok Product GMV Max (kolom Akun TikTok)','creator and video performance from TikTok Product GMV Max (TikTok Account column)')}</span></h2>
  <div class="controls">
  ...
  <h2 class="sec">${L('Video milik ','Videos by ')}${esc(sel.creator)} <span class="hint">${sel.vids.size} ${L('video · diurutkan berdasarkan nilai','videos · sorted by value')}</span></h2>
  <div class="tbl-wrap" style="max-height:560px;overflow:auto"><table><thead><tr>
  ...
  <h2 class="sec">${L('Daftar Kreator','Creator List')} <span class="hint">${L('klik baris untuk melihat videonya','click a row to view its videos')}</span></h2>
  <div class="tbl-wrap"><table><thead><tr>
...
function botReply(q0){
  ...
  for(const p in PLATFORMS){
    if(q.includes(p)){
      ...
      return `<b>${PLATFORMS[p].name} · ${mkLabel(LATEST)} (MTD)</b>\n`+
        L(`Spend penjualan ${fmtRpC(s.cost)}${u.cost?` · upper funnel ${fmtRpC(u.cost)}`:''}\nGMV ${fmtRpC(s.gmv)} · ROAS ${fmtX(s.roas)} · ${fmtN(s.orders)} pesanan\n`,
          `Sales spend ${fmtRpC(s.cost)}${u.cost?` · upper funnel ${fmtRpC(u.cost)}`:''}\nGMV ${fmtRpC(s.gmv)} · ROAS ${fmtX(s.roas)} · ${fmtN(s.orders)} orders\n`)+
  ...
  if(has('spend','biaya','anggaran','budget','cost','pacing')){
    ...
    return `<b>Spend ${mkLabel(LATEST)} (MTD)</b>\n`+
      L(`Total ${fmtRpC(st.spend)} (penjualan ${fmtRpC(st.spendSales)} · upper ${fmtRpC(st.spendUpper)})\nAnggaran channel tercatat ${fmtRpC(totB)} · terpakai ${fmtRpC(totS)}\n`,
        `Total ${fmtRpC(st.spend)} (sales ${fmtRpC(st.spendSales)} · upper ${fmtRpC(st.spendUpper)})\nRecorded channel budgets ${fmtRpC(totB)} · used ${fmtRpC(totS)}\n`)+
