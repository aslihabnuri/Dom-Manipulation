...
def build(photo_path,out_path,k=1.0,bigw=400,hero_y=0.215,eyebrow="PAYDAY SALE",big="45%",benefits=("Free shipping","Extra IDR 5K voucher","for new buyers")):
    ...
    f_big=fit_font(d,big,"ZalandoSansExpanded-Black.ttf",g(bigw),track=g(-8))
    y=int(H*hero_y)
    w=tracked_w(d,eyebrow,f_eye,g(8)); tracked(d,((W-w)//2,y),eyebrow,f_eye,WHITE,track=g(8)); y+=g(26+22)
    w=tracked_w(d,"SAVE UP TO",f_save,g(10)); tracked(d,((W-w)//2,y),"SAVE UP TO",f_save,WHITE,track=g(10)); y+=g(46+12)
    bb=d.textbbox((0,0),big,font=f_big); tracked(d,((W-(bb[2]-bb[0]))//2-bb[0],y-bb[1]),big,f_big,WHITE,track=g(-8))
    y+= (bb[3]-bb[1]) + g(30)
    # benefits directly under the hero, still one centred block
    f_ben=font("Arimo-Regular.ttf",28); f_cta=font("ZalandoSansExpanded-Bold.ttf",28); f_tc=font("Arimo-Regular.ttf",20)
    for t in benefits:
        d.text(((W-d.textlength(t,font=f_ben))//2,y),t,font=f_ben,fill=WHITE); y+=g(28+10)
...
if __name__=="__main__":
    photo,out=sys.argv[1],sys.argv[2]; k=float(sys.argv[3]) if len(sys.argv)>3 else 1.0; hy=float(sys.argv[4]) if len(sys.argv)>4 else 0.215; bw=int(sys.argv[5]) if len(sys.argv)>5 else 400
    im=build(photo,out,k=k,hero_y=hy,bigw=bw); im.resize((540,810),Image.LANCZOS).save(out.rsplit(".",1)[0]+"_preview.jpg",quality=85); print(out,im.size)
